...
subroutine ice_deposition_sublimation(t, qv, qi, ni, &
                                      ...
                                      Do_CldLiq, Do_CldIce, &  ! Jamison A Smith (JAS) 2020 Aug 22
  ...
  logical,                     intent (in) :: Do_CldLiq  ! Jamison A Smith (JAS) 2020 Aug 22
  logical,                     intent (in) :: Do_CldIce  ! Jamison A Smith (JAS) 2020 Oct 10
  ...
  do i=1,mgncol
     if (qi(i)>=qsmall) then
        ...
        !  PC: sublimation occurs @ any T. Not so for berg.
        
        !  JAS 2020 Aug 22: Only do the Bergeron Process if MG is doing
        ! liquid and ice calculations

        if ( Do_CldLiq .and. Do_CldIce .and. t(i) < tmelt) then  ! JAS 2020 Aug 22

           !Compute bergeron rate assuming cloud for whole step.
           berg(i) = max(epsi/ab*(qvl(i) - qvi(i)), 0._r8)
        else !T>frz
           berg(i)=0._r8
        end if  ! Do_CldLiq .and. Do_CldIce .and. t(i) < tmelt
...
subroutine evaporate_sublimate_precip(t, rho, dv, mu, sc, q, qvl, qvi, &
  ...
  logical,                     intent (in) :: Do_CldLiq, Do_CldIce  ! Jamison A Smith (JAS) 2020 Aug 22
  ...
  ! only calculate if there is some precip fraction > cloud fraction
     ...
     if (precip_frac(i) > dum(i)) then
        ...
        ! evaporation of rain
        if ( Do_CldLiq .and. qric(i) >= qsmall) then
        ...
        ! sublimation of snow
        if ( Do_CldIce .and. qsic(i) >= qsmall) then
           ab = calc_ab(t(i), qvi(i), xxls)
