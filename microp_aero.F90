...
use hetfrz_classnuc_cam, only: hetfrz_classnuc_cam_readnl, hetfrz_classnuc_cam_register, hetfrz_classnuc_cam_init, &
                               hetfrz_classnuc_cam_save_cbaero, hetfrz_classnuc_cam_calc,hetfrz_classnuc_cam_cbaero
...
subroutine microp_aero_run ( &
   ...
   !  Jamison A Smith (JAS) 2020 Oct 31: Turn off Cesm nucleation.
   ! Allow the code of Charles Bardeen known as Carma Cirrus to freeze
   ! ice on aerosol particles.

   If ( .false. ) Then

    write (*,*) 'Jamison A Smith (JAS) 2020 Oct 31 Unwanted ice nucleation'
    !cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
    !ICE Nucleation
   ...
   if (clim_modal_aero) then
      ...
      !+++ Cheng: test npccn_scale
      !npccn(:ncol,:) = nctend_mixnuc(:ncol,:)
      npccn(:ncol,:) = nctend_mixnuc(:ncol,:) * 1.5_r8
   ...
   !  Jamison A Smith (JAS) 2020 Oct 31: Turn off Cesm contact freezing.
   ! Allow the code of Charles Bardeen known as Carma Cirrus to freeze
   ! ice on aerosol particles.
   ...
   !  Jamison A Smith (JAS) 2020 Oct 31: Turn off Cesm heterogenous
   ! freezing.  Allow the code of Charles Bardeen known as Carma Cirrus to
   ! freeze ice on aerosol particles.

   If ( .false. ) Then     ! (todo) +++ Commented by Cheng-Cheng Liu: To double-check with Yunqian Zhu. To turn on/off.
    ...
    if (use_hetfrz_classnuc) then
      ...
      call hetfrz_classnuc_cam_cbaero(state1, deltatin, factnum, pbuf)
    end if
