...
subroutine hetfrz_classnuc_calc( &
   ...
   frzbccnt, frzducnt,                        &
   frzbcdep, frzdudep,                        &
   frzbcimm_jimm,frzduimm_jimm1,frzduimm_jimm2,               &
   frzbccnt_jimm, frzducnt_jimm1,frzducnt_jimm2,              &
   frzbcimm_pct,frzduimm_pct1,frzduimm_pct2,                 &
   frzbccnt_pct, frzducnt_pct1,frzducnt_pct2,                &
   ...
   ! zhu 20220909 add variables to pass to CARMA
   real(r8), intent(out) :: frzbcimm_jimm      ! het. frz by BC immersion nucleation probability [1]
   real(r8), intent(out) :: frzduimm_jimm1      ! het. frz by dust immersion nucleation probability[1]
   real(r8), intent(out) :: frzduimm_jimm2      ! het. frz by dust immersion nucleation probability[1]
   real(r8), intent(out) :: frzbccnt_jimm      ! het. frz by BC contact nucleation probability[1]
   real(r8), intent(out) :: frzducnt_jimm1      ! het. frz by dust contact nucleation probability[1]
   real(r8), intent(out) :: frzducnt_jimm2      ! het. frz by dust contact nucleation probability[1]
   real(r8), intent(out) :: frzbcimm_pct       ! immersion by BC percentage [1]
   real(r8), intent(out) :: frzduimm_pct1       ! immersion by dust percentage [1]
   real(r8), intent(out) :: frzduimm_pct2       ! immersion by dust percentage [1]
   real(r8), intent(out) :: frzbccnt_pct       ! contact by BC percentage [1]
   real(r8), intent(out) :: frzducnt_pct1       ! contact by dust percentage [1]
   real(r8), intent(out) :: frzducnt_pct2       ! contact by dust percentage [1]
   ...
   frzbcimm_jimm = 0._r8
   frzduimm_jimm1 = 0._r8
   frzduimm_jimm2 = 0._r8
   frzbccnt_jimm = 0._r8
   frzducnt_jimm1 = 0._r8
   frzducnt_jimm2 = 0._r8
   ...
   ! zhu 20220909
   if(.not.tot_in)then
     frzbcimm_pct = total_cloudborne_aer_num(id_bc)/(total_cloudborne_aer_num(id_bc) + &
                  total_cloudborne_aer_num(id_dst1)+total_cloudborne_aer_num(id_dst3))
     frzduimm_pct1 = total_cloudborne_aer_num(id_dst1)/(total_cloudborne_aer_num(id_bc) + &
                  total_cloudborne_aer_num(id_dst1)+total_cloudborne_aer_num(id_dst3))
     frzduimm_pct2 = total_cloudborne_aer_num(id_dst3)/(total_cloudborne_aer_num(id_bc) + &
                  total_cloudborne_aer_num(id_dst1)+total_cloudborne_aer_num(id_dst3))
   else
     frzbcimm_pct = fn(id_bc)*total_aer_num(id_bc)/(fn(id_bc)*total_aer_num(id_bc) + &
                  fn(id_dst1)*total_aer_num(id_dst1)+fn(id_dst3)*total_aer_num(id_dst3))
     frzduimm_pct1 = fn(id_dst1)*total_aer_num(id_dst1)/(fn(id_bc)*total_aer_num(id_bc) + &
                  fn(id_dst1)*total_aer_num(id_dst1)+fn(id_dst3)*total_aer_num(id_dst3))
     frzduimm_pct2 = fn(id_dst3)*total_aer_num(id_dst3)/(fn(id_bc)*total_aer_num(id_bc) + &
                  fn(id_dst1)*total_aer_num(id_dst1)+fn(id_dst3)*total_aer_num(id_dst3))
   end if
   ...
   if (.not.tot_in) then
      ...
      if (do_bc) frzbcimm_jimm = Jimm_bc
      if (.not. pdf_imm_in) then
         ...
         if (do_dst1) frzduimm_jimm1 = Jimm_dust_a1
         if (do_dst3) frzduimm_jimm2 = Jimm_dust_a3
      else
         if (do_dst1) frzduimm = frzduimm+MIN(1*total_cloudborne_aer_num(id_dst1)/deltat,        &
         ...
         if (do_dst1) frzduimm_jimm1 = -log(sum_imm_dust_a1)/deltat
         if (do_dst3) frzduimm_jimm2 = -log(sum_imm_dust_a3)/deltat
      end if
   else
