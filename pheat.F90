...
!  real(kind=f)                         :: qrad0                   ! particle net radiation (Tp=Ta) (erg/s)
  ...
  !Zhu 2022 01 25 we force akas = 1. to intialize the model
  ! so that the particle can go over the kolar curve
!  if (igroup == 2) akas = 1._f
...
!              (supsatl(iz,igas) .lt. 0._f) ) akas = 1._f
  ...
  if ((.not. do_pheat) .or. (.not. do_mie(igroup))) then
    ...
    !if(p(iz) .lt. 100000._f .and. igroup .eq. 4) dmdt = pvap*(-akas)*g0/(1._f+g0*g1*pvap)
...
!      end if
      ...
      dmdt = pvap * ( ss + 1._f - akas * (1._f + qrad * g1 * g2 )) * &
             g0 / ( 1._f + g0 * g1 * pvap )
      !if(p(iz) .lt. 100000._f .and. igroup .eq. 4)then
      !   dmdt = pvap * (- akas * (1._f + qrad * g1 * g2 )) * &
      !       g0 / ( 1._f + g0 * g1 * pvap )
      !end if
