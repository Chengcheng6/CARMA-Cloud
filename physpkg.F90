module physpkg
  ...
  ! 2019 Nov    J. A. Smith    Moving entry point for Carma Cloud model. Other
  !             (JAS)          Carma models are Carma Cirrus and Carma Aerosol.
  ! +++ 
  ! 2024-02-23 Cheng-Cheng Liu (Cheng) 
  !            Upgrade CARMA_cloud from CESM211 to CESM222 base code (e.g. snapshot).
  !            ! +++ for CARMA_cloud. {CARMA cloud codes}.
  !----------------------------------------------------------------------------
  ...
  ! +++ for CARMA_cloud
  use carma_flags_mod, only: carma_do_aerosol, carma_do_detrain, &
                             carma_do_cldice, carma_do_cldliq, &
                             carma_do_emission, carma_do_wetdep
  ...
  ! +++ for CARMA_cloud
  integer ::  NpCcn_idx          = 0     !  Activation rate of Mam aerosol from
                                         ! Mod ndrop.F90 Sub mixnucdrop
  integer ::  cmeliq_idx         = 0                                       
  integer ::  cmeice_idx         = 0                                       
  integer ::  qsatfac_idx        = 0
  
  ! Cloud fraction variables needed for qsatfac adjustment.
   real(r8) :: rhminl
   real(r8) :: rhminh
   real(r8) :: premit
   real(r8) :: premib
...
!=======================================================================
  ...
  subroutine phys_init( phys_state, phys_tend, pbuf2d, cam_in, cam_out )
    ...
    ! +++ In CARMA_cloud code, added cldfrc_getparams
    use cloud_fraction,     only: cldfrc_init, cldfrc_getparams
    ...
    ! +++ for CARMA_cloud
    ! Get cloud fraction settings for liquid cloud.
    call cldfrc_getparams(rhminl_out=rhminl, rhminh_out=rhminh, premit_out=premit, premib_out=premib)
    ...
    cmeice_idx = pbuf_get_index('CMEICE', ierr)
...
!$OMP PARALLEL DO PRIVATE (C, NCOL, phys_buffer_chunk)
  ...
  subroutine tphysac (ztodt,   cam_in,  &
    ...
    ! +++ for CARMA_cloud, added carma_output_budget_diagnostics
   ...
   !------------------------------------------
    ...
    if (do_clubb_sgs) then
      call carma_output_budget_diagnostics(state, ptend, ztodt, "VDIF")
    else
      call carma_output_budget_diagnostics(state, ptend, ztodt, "VDIF", flx_vap=cam_in%cflx(:,1), flx_sen=cam_in%shf)
    end if
    ! ! call physics_update(state, ptend, ztodt, tend)  ! Note: already called next in the CESM222
    ! +++ for CARMA_cloud
   ...
   if (carma_do_aerosol) then
    ...
    ! +++ for CARMA_cloud # if
    call carma_output_budget_diagnostics(state, ptend, ztodt, "GWTEND")
    ! +++ for CARMA_cloud # endif
...
!!!   call check_energy_chng(state, tend, "drymass", nstep, ztodt, zero, zero, zero, zero)
  ...
  subroutine tphysbc (ztodt, state,  &
    ...
    ! +++ for CARMA_cloud, added dtype_r8 and pbuf_add_field
    use physics_buffer,  only: physics_buffer_desc, pbuf_get_field, dtype_r8
    use physics_buffer,  only: pbuf_get_index, pbuf_old_tim_idx, pbuf_add_field
    ...
    ! +++ for CARMA_cloud. added carma_output_budget_diagnostics, carma_checkstate_global
    use carma_intr,      only: carma_wetdep_tend, carma_timestep_tend, &
        carma_output_budget_diagnostics, carma_checkstate_global
    ...
    ! +++ for CARMA_cloud {
    !   Activation rate of Mam aerosol from Mod ndrop Sub dropmixnuc. ActRate
    ! is a misnomer because it was named for NpCcn, which is also a misnomer.
    ! The change in the number mixing ratio of liquid water droplets number
    ! includes changes from cloud fraction and reinvigoration. - JAS, 2019

    real(r8) ActRate (pcols, pver)
    ! +++ for CARMA_cloud }
    ...
    ! ixq added both in CESM222 and CARMA_cloud
    ...
    ! +++ for CARMA_cloud {
    real(r8),pointer :: cmeliq(:,:)     ! liquid condensed by CLUBB
    real(r8),pointer :: cmeice(:,:)     ! ice condensed by CLUBB
    real(r8),pointer :: qsatfac(:,:)    ! subgrid saturation factor
    
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ! LW (08/2023): Add for determining precip and evap tendency
    ! CARMA prain, nevap pointer
    real(r8),pointer :: prain_ptr(:,:)           ! precip. rate
    real(r8),pointer :: evapr_ptr(:,:)           ! evap. rate for precip.
    real(r8),pointer :: prain_micmac_ptr(:,:)    ! precip. rate based on micmac step
    real(r8),pointer :: evapr_micmac_ptr(:,:)    ! evap. rate for precip. based on micmac step
    
    real(r8) :: sumprain(pcols,pver)           ! sum of precip. rate during CAM time step
    real(r8) :: sumevapr(pcols,pver)           ! sum of evap. rate of precip during CAM time step      
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ! +++ for CARMA_cloud }
    ...
    ! +++ for CARMA_cloud {
    real(r8) :: det_cnd(pcols)                 ! vertical integral of detrained ice
   ...
   ! +++ for CARMA_cloud }
    ...
    logical   :: lq(pcnst)
    ! +++ for CARMA_cloud
    logical   :: lq_crcme(pcnst)
    real(r8)  :: es, qs
    integer   :: icol, iz
    real(r8)  :: rhmin, rhwght

    ! +++ for CARMA_cloud {
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
! LW (May 2023): added for cloud budget diagnostics
...
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ß   
    ...
    ! +++ for CARMA_cloud
       call carma_output_budget_diagnostics(state, ptend, ztodt, "EFIX", flx_sen=flx_heat)
    ! +++ for CARMA_cloud
       call physics_update(state, ptend, ztodt, tend)
    ...
    call cnst_get_ind('CLDLIQ', ixcldliq)
    call cnst_get_ind('CLDICE', ixcldice)
    ! +++ for CARMA_cloud. Added both in CARMA_cloud and CESM222
    call cnst_get_ind('Q', ixq)         ! LW(Dec2022): added for supersat, to get gc_cgsold
    ...
    call carma_output_budget_diagnostics(state, ptend, ztodt, "CRGFIX")
    ...
    ! +++ for CARMA_cloud
    !++++++++
    call carma_output_budget_diagnostics(state, ptend, ztodt, "DRYADJ")
    !++++++++
    ...
    ! --- for CARMA_cloud. 
         ! Note: do physics_update and t_stopf('convect_deep_tend') next in CARMA_cloud codes.
    ...
    ! +++ Modified comments in CARMA cloud codes.
    flx_cnd(:ncol) = prec_dp(:ncol) + rliq(:ncol)
    snow_dp(:ncol) = snow_dp(:ncol) + rice(:ncol)

    ! -------------------------------------------------------------------------------------------------------
    ! +++ for CARMA_cloud. { 
    ! ! Merged by Cheng. 20240223
    ! ! Note: also for changes in CESM222.

    ! budget diag. added for CARMA_cloud
    call carma_output_budget_diagnostics(state, ptend, ztodt, "DEEP", flx_cnd=flx_cnd, flx_ice=snow_dp)
   ...
   ! ---------------------------------------------------------------------------------------------------------
    ...
    call carma_output_budget_diagnostics(state, ptend, ztodt, "SHALLOW", flx_cnd=flx_cnd, flx_ice=snow_sh)
    ! +++ for CARMA_cloud }
    ...
    ! --- for CARMA_cloud. Noted by Cheng. Already done in lines before calling carma_output_budget_diagnostics.
    ! flx_cnd(:ncol) = prec_sh(:ncol) + rliq2(:ncol)
...
! -----------------------------------------------------------------------------------------------------------------------
    !  --- for CARMA_cloud. Noted by Cheng. 
         ! But for now, I didn't know why JAS commented it out. 
         ! carma_do_cldice .or. carma_do_cldliq. Also, carma_do_detrain
    !  --- Below is commented by JAS.
    
    !  This entry point is for Carma Cirrus.  The entry point for Carma Cloud
    ! is below Clubb.  The entry point for Carma Aerosol is in Subroutine
    ! tphysac.  - JAS 2019
    ! 
    ! NOTE: It would be better to have the cirrus model work inside the macmic loop
    ! like the liquid/ice model. It may not play nicely with CLUBB as it stands, so
    ! just comment it out for now.

!    call t_startf('carma_timestep_tend')

!    if (carma_do_cldice .and. .not. carma_do_cldliq .and. .not. carma_do_aerosol) then
...
!         call carma_output_budget_diagnostics(state, ptend, ztodt, "CRTEND0", flx_cnd=prec_str+rliq, flx_ice=snow_str+rliq)
!       else
!         call carma_output_budget_diagnostics(state, ptend, ztodt, "CRTEND0", flx_cnd=prec_str, flx_ice=snow_str)
!       end if
      !++++++++
!       call physics_update(state, ptend, ztodt, tend)
...
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
       ! LW (08/2023): Add for determining precip and evap tendency
       ! zero out sumprain and sumevapr before the macmic loop
       sumprain(:,:) = 0._r8
       sumevapr(:,:) = 0._r8
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
       ...
       do macmic_it = 1, cld_macmic_num_steps
          ...
          !print*, 'Mod physpkg Sub tphysbc: begin  nstep  macmic_it: ', nstep &
          !, macmic_it
          ...
          if (macrop_scheme .ne. 'CLUBB_SGS') then
             ...
             ! +++ for CARMA_cloud. {
             flx_vap(:ncol) = cam_in%cflx(:ncol,1)
             flx_ice(:ncol) = det_ice(:ncol)
             !  +++ for CARMA_cloud. }
            ...
            ! +++ for CARMA_cloud.
             ...
             call carma_output_budget_diagnostics(state, ptend, ztodt, pname, &
               flx_vap=flx_vap/cld_macmic_num_steps, flx_cnd=flx_cnd/cld_macmic_num_steps, &
               flx_ice=flx_ice/cld_macmic_num_steps, flx_sen=flx_heat/cld_macmic_num_steps)
            ! +++ for CARMA_cloud.
          ...
          ! +++ for JAS's comments only.
          !  CLUBB (Sub clubb_tend_cam) has been called before Sub
          ! microp_aero_run. NPCCN is filled by microp_aero_run. Both names
          ! NPCCN and ActRate are misnomers. The tendency for droplet number
          ! mixing ratio is affected by droplet activation of Mam aerosol, but
          ! this tendency is also affected by changes in cloud fraction and
          ! cloud reinvigoration. - JAS 2019
          ...
          ! ---------------------------------------------------------------------   
          ! +++ for CARMA_cloud. { 
          else  ! use_subcol_microp

            !  clubb_cam_tend calls Clubb macrophysics to determine the cloud
            ! fraction, equivalent potential temperature, and CLDLIQ.
            !  NPCCN is filled by microp_aero_run. 
            !  MG microphysics processes the cloud particles and calculates the 
            ! radiative properties of those cloud particles. The call to Carma
            ! needs to occur in between Clubb and MG, and the MG processes need
            ! to be turned off when Carma is processing the cloud particles.

            call t_startf('carma_timestep_tend')

            if (carma_do_cldliq .and. .not. carma_do_aerosol) then
            
              ! Before calling CARMA, put the water that CLUBB condensed and
              ! detrained back into vapor so it can be recondensed onto the
              ! CARMA liquid and ice. The latent heat also needs to be
              ! restored.
              !
              ! NOTE: In the future, we may want to try to separate out the
              ! detrained ice (and liquid?) and redetrain it in CARMA. However,
              ! you have to be careful about whether the detrained condensate has
              ! been convected or evaporated, so you may not be able to just
              ! take it away from where deep convection put it.
              !
              ! NOTE: This could/should be done in CARMA, but for now let's do it
              ! here and treat it as a physics package so we can see it in
              ! package diagnostics.
              ! 
              ! NOTE: To make sure that CARMA recondenses the same amount of
              ! liquid as CLUBB condensed, the QSATFAC may need to be adjusted.
              ...
              lq_crcme(:)        = .false.
              lq_crcme(ixq)      = .true.
              lq_crcme(ixcldliq) = .true.
              lq_crcme(ixcldice) = .true.
              
              call pbuf_get_field(pbuf, cmeliq_idx, cmeliq)
              call pbuf_get_field(pbuf, cmeice_idx, cmeice)
              call pbuf_get_field(pbuf, qsatfac_idx, qsatfac)
          
              call physics_ptend_init(ptend, state%psetcols, 'CRCME', ls=.true., lq=lq_crcme)
              
              ptend%q(:ncol,:,ixq) = cmeliq(:ncol,:) + cmeice(:ncol,:)
              ptend%q(:ncol,:,ixcldliq) = -cmeliq(:ncol, :)
              ptend%q(:ncol,:,ixcldice) = -cmeice(:ncol, :)
              
              ! NOTE: ptend%s is in J/kg/s, latvap is J/kg, and cmeliq is kg/kg/s
              ptend%s(:ncol,:) = (-cmeliq(:ncol,:) * latvap - cmeice(:ncol, :) * (latvap + latice))             

              ! Adjust qsatfac so the it results in the same final amount of
              ! cldliq as CLUBB,              
              do icol = 1, ncol
                do iz = 1, pver
                
                  ! Adjust the subgrid scale saturation factor so that the liquid
                  ! saturation * qsatfac is 1 where cloud liquid growth or
                  ! evaporation happened.
                  !
                  ! NOTE: If all the liquid evaporated, then the final saturation
                  ! ratio could be less then 1, so don't have qsatfac force it to
                  ! 1 in that case. However, if there is some liquid left then
                  ! the liquid saturation at that point should be 1.
                  !
                  ! NOTE: CMELIQ appears to include condensate in places that are not
                  ! very close to liquid saturation. They seem to occur with negative
                  ! values of cmeliq where the saturation is low and not all the
                  ! condensate is removed.
                  if (cmeliq(icol,iz) .gt. 0._r8) then
              !    if ((cmeliq(icol,iz) .gt. 0._r8) .or. &
              !        ((cmeliq(icol, iz) .lt. 0._r8) .and. (state%q(icol,iz,ixcldliq) .gt. qmin(ixcldliq)))) then
                    call wv_sat_qsat_water(state%t(icol,iz), state%pmid(icol,iz), es, qs)
                    qsatfac(icol, iz) = min(state%q(icol,iz,ixq) / qs, 1._r8)
                    
                    ! Make the minimum value for qsatfac be the RH for the onset of
                    ! liquid clouds, which varies from rhminl to rhminh depending on
                    ! the pressure level.
                    !
                    ! NOTE: This logic is from cldfrc2m.
                    if (state%pmid(icol,iz) .lt. premib) then
                      rhmin = rhminl
                    else if (state%pmid(icol,iz) .lt. premit) then
                      rhmin = rhminh
                    else
                      rhwght = (premib - (max(state%pmid(icol,iz), premit))) / (premib-premit)
                      rhmin = rhminh*rhwght + rhminl*(1.0_r8-rhwght)
                    end if
                    
                    qsatfac(icol, iz) = max(qsatfac(icol, iz), rhmin)
                  end if
                end do
              end do
              ...
              write(pname, '(A, I2.2)') "CRCME", macmic_it
              call carma_output_budget_diagnostics(state, ptend, ztodt, pname)

              call physics_update (state, ptend, ztodt, tend)

              call check_energy_chng (state, tend, "crcme_tend", nstep, ztodt, zero, zero, zero, zero)
              
              
              ! Since CARMA is being called in a loop, the precipitation
              ! needs to be accumulated.
              call carma_timestep_tend(state, cam_in, cam_out, ptend, cld_macmic_ztodt, &
                                       pbuf, dlf=dlf, det_cnd=det_cnd, det_ice=det_ice, &
                                       prec_sed=prec_sed_carma, snow_sed=snow_sed_carma)

              call physics_ptend_scale(ptend, 1._r8/cld_macmic_num_steps, ncol)
              
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
              ! LW (08/2023): Add for determining precip and evap tendency
              ! get 10-min pbuf for CARMA prain, nevapr
              call pbuf_get_field(pbuf, pbuf_get_index('CRPRAIN_micmac'), prain_micmac_ptr)
              call pbuf_get_field(pbuf, pbuf_get_index('CRNEVAPR_micmac'), evapr_micmac_ptr)
              
              sumprain(:ncol,:) = sumprain(:ncol,:) + prain_micmac_ptr(:ncol,:)
              sumevapr(:ncol,:) = sumevapr(:ncol,:) + evapr_micmac_ptr(:ncol,:)
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
              
              write(pname, '(A, I2.2)') "CRTEND", macmic_it
              
              ! NOTE: Since CLUBB has already detrained in CLDLIQ and CLDICE, we don't need to
              ! account for the ice being added, we are putting it into CRLIQ and CRxICE, which
              ! will then update CLDLIQ and CLDICE. We just need to account for the precipitation.
!              flx_cnd(:ncol) = (prec_sed_carma(:ncol) - det_cnd(:ncol))/cld_macmic_num_steps
!              flx_ice(:ncol) = (snow_sed_carma(:ncol) - det_ice(:ncol))/cld_macmic_num_steps
              flx_cnd(:ncol) = prec_sed_carma(:ncol)/cld_macmic_num_steps
              flx_ice(:ncol) = snow_sed_carma(:ncol)/cld_macmic_num_steps
              call carma_output_budget_diagnostics(state, ptend, ztodt, pname, flx_cnd=flx_cnd, flx_ice=flx_ice)
              !++++++++
              
              call physics_update (state, ptend, ztodt, tend)

              call check_energy_chng (state, tend, "carma_tend", nstep, ztodt, &
                 zero, flx_cnd, flx_ice, zero)

              call t_stopf('carma_timestep_tend')
            endif

            ! - JAS: The Carma Cloud Entry Point 
            !--
            !  JAS: Now, call MG to allow for the calculation of the radiative
            ! properties of the Carma Cloud Particles. The flags do_cldliq and
            ! do_cldice (with a scope local to micro_mg_tend) will direct MG to
            ! skip cloud processing.
             call microp_driver_tend(state, ptend, cld_macmic_ztodt, pbuf)
          end if  ! use_subcol_microp
          ! +++ for CARMA_cloud. }
          ! ---------------------------------------------------------------------
         ...
         ! +++ for CARMA_cloud. {
          ...
          flx_cnd(:ncol) = prec_str(:ncol)/cld_macmic_num_steps
          flx_ice(:ncol) = snow_str(:ncol)/cld_macmic_num_steps
          call carma_output_budget_diagnostics(state, ptend, ztodt, pname, flx_cnd=flx_cnd, flx_ice=flx_ice)
         ! +++ for CARMA_cloud. }
          ...
          if (carma_do_cldice .or. carma_do_cldliq) then
            prec_sed_macmic(:ncol) = prec_sed_macmic(:ncol) + prec_sed_carma(:ncol)
            snow_sed_macmic(:ncol) = snow_sed_macmic(:ncol) + snow_sed_carma(:ncol)
          end if
          ! +++ for CARMA_cloud. }
       ...
       !++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
       ! LW (08/2023): Add for determining precip and evap tendency
       call pbuf_get_field(pbuf, pbuf_get_index('PRAIN'), prain_ptr )
       call pbuf_get_field(pbuf, pbuf_get_index('NEVAPR'), evapr_ptr )
      
       prain_ptr(:ncol,:) = sumprain(:ncol,:) / cld_macmic_num_steps
       evapr_ptr(:ncol,:) = sumevapr(:ncol,:) / cld_macmic_num_steps
       
       call outfld('PRODPREC',    prain_ptr,      pcols, lchnk)
       call outfld('EVAPPREC',    evapr_ptr,      pcols, lchnk)             
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
       ! +++ for CARMA_cloud. }
    ...
    endif  ! Comments only in CARMA_cloud: microp_scheme == 'RK' or 'MG'

    ! --- for CARMA_cloud. Moved to above 
    ...
    if ( .not. deep_scheme_does_scav_trans() ) then
      ...
      !+++ for CARMA_cloud.
       call carma_output_budget_diagnostics(state, ptend, ztodt, "WETDEPA")
      !+++ for CARMA_cloud. #
       ...
       if (carma_do_wetdep) then
          ...
          !+++ for CARMA_cloud.
          call carma_output_budget_diagnostics(state, ptend, ztodt, "WETDEPC")
          !+++ for CARMA_cloud. #
       ...
       ! +++ for CARMA_cloud.
       call carma_output_budget_diagnostics(state, ptend, ztodt, "DEEP2")
       ! +++ for CARMA_cloud.
