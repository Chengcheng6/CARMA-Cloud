...
!                    ! +++   
!                    Feb 2024: Cheng-Cheng Liu (Cheng)
!                              Upgrate our CARMA_cloud from base code of mg2 to pumas/micro_mg3_0.F90
!                              ! MG3 mainly added hail and graupel
!                    Aril 2024: Chuck and Cheng: DEBUG fixed  
!                    ! +++    
...
!                    Oct 2020: Merged with Carma Cirrus for high level bin
!                              clouds and Cloud for low level bin clouds
!                              Jamison A Smith (JAS)
!
...
!
! Charles "Chuck" Bardeen (CB) circa 2010
!
...
!
! ! Jamison A Smith (JAS) is working on this 2019 - 2020
! More comments appended at the end of this header of comments
...
!==============================================================================
! Author:                Jamison A Smith
! Version carma cirrus:  2020 Oct:  Merge MG 2.0 with CARMA
!                                   Improve the use of Do_CldLiq and Do_CLdIce
!                                   Work with eight MG prognostic condensate
!                                   vars:
!
!                                   1) CLDLIQ  3) CLDICE  5) RAINQM  7) SNOWQM
!                                   2) NUMLIQ  4) NUMICE  6) NUMRAI  8) NUMSNO
!
! Community Aerosol and Radiation Model for Atmospheres (CARMA)
! Bin microphysics model of aerosol and clouds
! Also known as the NASA Ames Stratospheric Aerosol Model
! Many developers: O. B. Toon, R. P. Turco, D. Westphal
!                  M. Z. Jacobsen, B. McKie, A. S. Ackerman, E. J. Jensen
!                  P. R. Colarco, J. A. Smith, A. M. da Silva, C. Bardeen
!                  C. Malone, Y. Zhu, P. Yu
!
! Tasks: Done 1) Zero liquid and ice tendencies
!        Done 2) Calculate gamma and lambda from pristine values for Radiation
!
!  By pristine values, I mean the values of mass mixing ratio and number mixing
! ratio of condensate that are given to MG.  I want to prohibit MG from
! processing the condensate, but I want MG to calculate the radiation vars from
! the mass mixing ratios and number mixing ratios that are fed to MG.
!
!        3) Add a switch for MG sedimentation to demonstrate that MG sed is the
!           the origin of the deep vertical extent of the boundary layer
!           clouds; an alternative hypothesis: vertical mixing
!        4) Turn off unnecesary calculations
!           a) Assume that nearly all of the processes involve three phases
!              except autoconversion, liq-rain accretion, and ice-snow accretion
!           b) Turn off nearly all processes with Do_CldLiq and Do_CldIce.  If I
!              make a mistake, I can enable that code later.  Stop thinking so
!              much; I have spent eighteen months repairing this module.
!==============================================================================
...
! Ice number sublimation parameter. Assume some decrease in ice number with
! sublimation if non-zero. Else, no decrease in number with sublimation. 
...
!logical :: do_cldice            ! +++ for CESM/MG2/MG3 base code.
logical :: Do_CldLiq, Do_CldIce  ! Jamison A Smith (JAS) 2020 Aug 21
logical :: use_hetfrz_classnuc
logical :: do_hail         ! +++ added for MG3.
logical :: do_graupel      ! +++ added for MG3.

real(r8) :: rhosu       ! typical 850 mb air density
...
subroutine micro_mg_init( &
     ...
     micro_mg_do_hail_in,micro_mg_do_graupel_in, &         ! +++ added for MG3.
     microp_uniform_in, Do_CldLiq_In, do_cldice_in, use_hetfrz_classnuc_in, &    ! +++ added Do_CldLiq_In for our CARMA_cloud.
     micro_mg_precip_frac_method_in, micro_mg_berg_eff_factor_in, &
     allow_sed_supersat_in, do_sb_physics_in, &
     nccons_in, nicons_in, ncnst_in, ninst_in, ngcons_in, ngnst_in, errstring)  !  +++ added ng* for MG3.
...
!MG3 dense precipitating ice. Note, only 1 can be true, or both false.
  ...
  logical,  intent(in)  :: Do_CldLiq_In     ! .true. = do all processes (standard)  JAS 2020 Aug 21
                                            ! .false. = skip all processes affecting
                                            !           Cloud Liquid and Rain
  ...
  Do_CldLiq  = Do_CldLiq_In                     ! JAS 2020 Aug 21
...
subroutine micro_mg_tend ( &
  ...
  !  JAS 2020 Mar 26: There is a conflict of intent for number mixing ratio
  ! of cloud water between micro_mg_timestep_tend and size_dist_param. Create
  ! a variable to work around this.
  ! ActArg is short for ActualArgument           ! cloud water number conc (1/kg)
  ...
  real(r8), intent(in) :: re_liq(:,:)    ! JAS 2020 Feb 24: liq effective radius (m)
  ...
  real(r8) :: dumb                ! JAS: I disapprove of these variable names
...
!need to zero these out to be totally switchable (for conservation)
  ...
  ! *->  JAS 2020 Oct 31: Has Carma already incremented NUMLIQ?  No.
  ! 1) Clubb  2) MG  3) Carma Cloud
  !--------------------------------------------------
 ...
 if ( Do_CldIce ) then
  ...
  if (do_cldice) then
     ...
     nnuccd = 0._r8  ! JAS 2019 Aug Uninitialized vars produce NaN when do_cldice = .false.
     nimax = 0._r8   ! If do_cldice = .false. then there will not be nucleation of ice
     mnuccd = 0._r8  ! so the number nuc rate, the max nuc number, and the mass nuc rate are zero
  end if
  ...
  micro_vert_loop: do k=1,nlev
     ...
     if (do_sb_physics) then
      ! +++ Cheng: both MG3 and CARMA_cloud changed this function. Be careful here! 
      ! +++ Cheng: No. JAS just added some spaces, no functional changes actually. So just use MG3's version here.
     ...
     if (do_cldice .and. Do_CldLiq ) then    
        ...
        end if  ! .not. use_hetfrz_classnuc

     else  ! do_cldice .and. Do_CldLiq
        mnuccc(:,k)=0._r8
        nnuccc(:,k)=0._r8
        mnucct(:,k)=0._r8
        nnucct(:,k)=0._r8
        mnudep(:,k)=0._r8
        nnudep(:,k)=0._r8
     end if  ! do_cldice .and. Do_CldLiq
   
   ! +++ Cheng: re-aligned for if conditions, based on JAS's code.  
   If ( Do_CldIce ) Then  ! JAS 2020 Aug 21
   ...
   EndIf  ! Do_CldIce     ! JAS 2020 Aug 21

     if ( Do_CldLiq .and. do_cldice) then   ! JAS 2020 Aug 22
        call secondary_ice_production(t(:,k), psacws(:,k), msacwi(:,k), nsacwi(:,k), mgncol)
     else
        psacws (:,k) = 0.0_r8  ! JAS 2020 Aug 21: Precip mass mix ratio secondary ice from
                               ! accretion of cloud water by snow.  No collection of cloud liquid
                               ! by snow
        nsacwi (:,k) = 0.0_r8  ! Number conc secondary ice from accretion of cloud water and ice
        msacwi (:,k) = 0.0_r8  ! Mass mix ratio secondary ice from accretion of cloud water and ice
     end if  ! Do_CldLiq .and. do_cldice    ! JAS 2020 Aug 22

     If ( Do_CldLiq .and. Do_CldIce ) Then  ! JAS 2020 Aug 21
     ...
     Else
      pracs  (:,k) = 0._r8  ! JAS 2020 Aug 22
      npracs (:,k) = 0._r8  ! JAS 2020 Aug 22
      mnuccr (:,k) = 0._r8  ! JAS 2020 Aug 22
      nnuccr (:,k) = 0._r8  ! JAS 2020 Aug 22
     EndIf  ! Do_CldLiq .and. Do_CldIce     ! JAS 2020 Aug 21

     If ( Do_CldLiq ) Then                  ! JAS 2020 Aug 21
      if (do_sb_physics) then
     ...
     Else                                   ! JAS 2020 Aug 22
      pra   (:,k) = 0._r8
      npra  (:,k) = 0._r8
      nragg (:,k) = 0._r8
     EndIf  ! Do_CldLiq                     ! JAS 2020 Aug 21
     ...
     call evaporate_sublimate_precip(t(:,k), rho(:,k), &
          ...
          qric(:,k), qsic(:,k), lamr(:,k), n0r(:,k), lams(:,k), n0s(:,k), &
          Do_CldLiq, Do_CldIce,                                                &  ! JAS 2020 Aug 22
          pre(:,k), prds(:,k), am_evp_st(:,k), mgncol)

     !  JAS 2020 Oct 10: Evap of rain out of cloud

     If ( .not. Do_CldLiq ) pre = 0._r8

     !  JAS 2020 Oct 10: Sublim of snow out of cloud

     If ( .not. Do_CldIce ) prds = 0._r8

     !  JAS 2020 Oct 10: am_evp_st is the fraction for evap, that is calc'd
     ! from cloud fraction and precip fraction.  I do not need to overwrite
     ! it.
     ...
     Else
      bergs (:,k) = 0._r8
     Endif  ! Do_CldLiq .and. Do_CldIce     ! JAS 2020 Aug 22

     !+++PMC 12/3/12 - NEW VAPOR DEP/SUBLIMATION GOES HERE!!!

     !  JAS 2020: Is PMC = Polar Mesospheric Cloud?  P H Caldwell wrote the
     ! Subroutine ice_deposition_sublimation.  PHC ~ PMC so I am confused.

     if ( Do_CldLiq .and. do_cldice ) then   ! JAS 2020 Oct 03

        call ice_deposition_sublimation(t(:,k), q(:,k), qi(:,k), ni(:,k), &
             icldm(:,k), rho(:,k), dv(:,k), qvl(:,k), qvi(:,k)          , &
             Do_CldLiq,                                                        &  ! JAS 2020 Aug 22
             Do_CldIce,                                                        &  ! JAS 2020 Oct 10
     ...
     Else
         !  JAS 2019 Aug Uninitialized vars produces NaN in static energy when
         ! do_cldice = .false.  Subroutine ice_deposition_sublimation assigns
         ! 0._r8 if qi(i) <= qsmall, so I can do the same.

         Berg       (:,k) = 0._r8
         Vap_Dep    (:,k) = 0._r8
         Ice_Sublim (:,k) = 0._r8
         nsubc      (:,k) = 0._r8  ! # conc cloud liq evap rate
     end if  ! Do_CldLiq .and. do_cldice
     !---PMC 12/3/12
...
!    pracs, npracs, (accretion of rain by snow)  psacr (collection of snow by rain)
        ...
        ! +++ Cheng: They new Addded in the MG3 for Graupel. 
        ! But seems it is duplicated with our CARMA cloud. We called above based on CESM211 MG2.
        ! Routine without Graupel (original->match CARMA_cloud)   
        call evaporate_sublimate_precip(t(:,k), rho(:,k), &
          dv(:,k), mu(:,k), sc(:,k), q(:,k), qvl(:,k), qvi(:,k), &
          lcldm(:,k), precip_frac(:,k), arn(:,k), asn(:,k), qcic(1:mgncol,k), qiic(:,k), &
          qric(:,k), qsic(:,k), lamr(:,k), n0r(:,k), lams(:,k), n0s(:,k), &
          Do_CldLiq, Do_CldIce, &   ! +++ Cheng: added here to let MG3 sync with our CARMA_cloud. 
          pre(:,k), prds(:,k), am_evp_st(:,k), mgncol)


     end if ! end do_graupel/hail loop

    If ( Do_CldLiq .and. Do_CldIce ) Then
     do i=1,mgncol
    ...
    Else
       If ( Do_CldLiq ) Then                               ! For Carma Cirrus
         do i=1,mgncol

           ! JAS 2020 Oct 10
           !
           ! 1) prc is a positive number ~ qcic ** 2.47 / ncic ** 1.1
           !    prod rate of rain mass mix ratio from autoconv of cloud liq
           ! 2) nprc is prod rate of number conc of rain from autoconv of cloud liq
           !    assume r of 25 um
           ! 3) nprc1 is a positive number = prc * ncic / qcic ~ qcic ** 1.47 / ncic ** 0.1
           !    loss rate of cloud liq number conc
           !
           !  If the loss of cloud liq from autoconversion and accretion is
           ! large, scale the loss to nearly consume nc.  prc is the prod rate
           ! of mass mix ratio of rain.  prc is also the loss rate of mass mix
           ! ratio of cloud water.  The same semantics apply to accretion and
           ! pra.

           dum = ( prc(i,k) + pra(i,k) ) * lcldm (i,k) * deltat
             if ( dum .gt. qc(i,k) ) then
    ...
    Else
      If ( Do_CldLiq ) Then
        ...
        ! JAS 2020 Oct 10: Notes from Sub kk2000_liq_autoconversion
        !
        ! 1) prc is a positive number ~ qcic ** 2.47 / ncic ** 1.1
        !    prod rate of rain mass mix ratio from autoconv of cloud liq
        ! 2) nprc is prod rate of number conc of rain from autoconv of cloud liq
        ...
        Do i = 1, mgncol
          ...
          !  JAS 2020 Oct 10: I can not find an example of using nsubc.
          ! It has been set to zero everywhere; I will reset it to zero
          ! in case someone starts using it at a later date because I do not
          ! know how to use nsubc correctly.
          !  If the loss of cloud liq from autoconversion and accretion is
          ! large, scale the loss to nearly consume nc

          nsubc (i,k) = 0._r8
          dum = ( nprc1(i,k) + npra(i,k) - nsubc(i,k) ) * lcldm (i,k) * deltat
          If ( dum .ge. nc(i,k) ) Then
    ...
    Else
       If ( Do_CldLiq ) Then
         do i=1,mgncol
           ...
           !  JAS 2020 Oct 10: pre is a positive number from
           ! Sub evaporate_sublimate_precip
...
!                 ( pra(i,k) + prc(i,k) ) * lcldm(i,k) ) * deltat
!
!           JAS: This looks incorrect: - pre is always less than QSmall
!
!           if ( dum .gt. qr(i,k) &
!                .and. &
!                -pre(i,k) .ge. QSmall ) then
!
!           ratio = ( qr(i,k) / deltat &
!                     + ( pra(i,k) + prc(i,k) ) * lcldm(i,k) &
!                   ) &
!                   / precip_frac (i,k) &
!                   / - pre(i,k) * omsm
!
...
!         endif
           ...
           !  JAS 2020 Oct 10: Evaporate outside of cloud
           ! I can not believe that I am using a variable named dum.  Ugh.

           !dum = pre (i,k) * max ( 0._r8, ( precip_frac (i,k) - lcldm (i,k) ) ) * deltat

           !  JAS 2020 Oct 10: This is the gridbox evaporation mass mix ratio

           dumb = abs ( pre (i,k) ) * max ( 0._r8, ( precip_frac (i,k) - lcldm (i,k) ) ) * deltat

           if ( dumb .gt. qr (i,k) ) pre (i,k) = pre (i,k) * OMSM * qr(i,k) / dumb

           nsubr(i,k) = 0._r8

           dumb = ( abs ( nragg (i,k) ) * precip_frac (i,k) &
                    + abs ( nsubr (i,k) ) * max ( 0._r8, ( precip_frac (i,k) - lcldm (i,k) ) ) &
                  ) * deltat

           if ( dumb .gt. nr (i,k) ) then
             nragg (i,k) = nragg (i,k) * OMSM * nr(i,k) / dumb
             nsubr (i,k) = nsubr (i,k) * OMSM * nr(i,k) / dumb
           EndIf
         enddo  ! i = 1, mgncol
       Endif  ! Do_CldLiq
     Endif  ! Do_CldLiq .and. Do_CldIce
     ...
     !  JAS 2020 Oct 10: I get lost.  The micro_vert_loop for k ends at line 2172.
     ...
     If ( Do_CldLiq .and. Do_CldIce ) Then
      do i=1,mgncol
        ...
        end if  ! (pre(i,k)+prds(i,k))*precip_frac(i,k)+ice_sublim(i,k) < -1.e-20_r8
     ...
     if ( Do_CldLiq .and. do_cldice ) then
      do i=1,mgncol
        ...
        if (do_cldice) then      ! +++ Cheng: JAS deleted this, but actually we can just keep it.
     ...
     Else
       If ( Do_CldLiq ) Then                ! For the Carma Cirrus simulation
         Do i = 1, mgncol

           qctend(i,k) = qctend(i,k) - ( pra(i,k) + prc(i,k) ) * lcldm (i,k)

           !  JAS 2020 Oct 10: There is no example of nsubc being applied
           ! so it is difficult to apply it correctly

           nctend(i,k) = nctend(i,k) &
                         + ( nsubc (i,k) - nprc1 (i,k) - npra (i,k) ) * lcldm (i,k)

           ! change to make sure prain is positive: do not remove snow from
           ! prain used for wet deposition

           prain(i,k) = ( pra(i,k) &                     ! accretion of cloud liq and rain
                          + prc(i,k) &                   ! autoconv of cloud liq to prod rain
                        ) * lcldm (i,k)

                        !+ ( - pracs   (i,k) &           ! coll of rain by snow
                        !    - mnuccr  (i,k) &           ! het freezing of rain
                        !    - mnuccri (i,k) &           ! freezing of rain to form cloud ice
                        !   ) * precip_frac (i,k)

           !  following are used to calculate 1st order conversion rate of cloud water
           ! to rain and snow (1/s), for later use in aerosol wet removal routine
           ! previously, wetdepa used (prain/qc) for this, and the qc in
           ! wetdepa may be smaller than the qc used to calculate pra, prc, ...
           !  in this routine
           ! qcsinksum_rate1ord = { rate of direct transfer of cloud water to rain & snow }
           !                      (no cloud ice or bergeron terms)

           qcsinksum_rate1ord(i,k) = ( pra(i,k) &         ! accretion of cloud liq and rain
                                       + prc(i,k) &       ! autoconv of cloud liq to prod rain
                                       ! +psacws (i,k) &  ! accretion of cloud liq and snow
                                     ) * lcldm (i,k)
  ...
  nc = ncn
  If ( Do_CldLiq ) nctend = nctend + npccn                                        ! JAS 2020 Aug 21

  ! Re-apply rain freezing and snow melting.
  dum_2D = qs
  qs = qsn
  If ( Do_CldIce ) qstend = qstend + (dum_2D-qs)/deltat                           ! JAS

  dum_2D = ns
  ns = nsn
  If ( Do_CldIce ) nstend = nstend + (dum_2D-ns)/deltat                           ! JAS

  dum_2D = qr
  qr = qrn
  If ( Do_CldLiq ) qrtend = qrtend + (dum_2D-qr)/deltat                           ! JAS

  dum_2D = nr
  nr = nrn
  If ( Do_CldLiq ) nrtend = nrtend + (dum_2D-nr)/deltat                           ! JAS
...
! Re-apply graupel freezing/melting
  ...
  If ( Do_CldLiq .and. Do_CldIce ) Then  ! Jamison A Smith (JAS) 2020 Aug 21
  ...
  Else
    ! Do not add evapsnow to nevapr
    ! Do not add prodsnow to prain
  EndIf  ! Do_CldLiq .and. Do_CldIce     ! Jamison A Smith (JAS) 2020 Aug 21

  do k=1,nlev
     do i=1,mgncol

       !  JAS 2020 Oct 03:  Allow update due to autoconversion, liq-rain
       ! accretion, and ice-snow aggregation
       ...
       If ( Do_CldLiq ) Then
        ...
        !dumi(i,k) = (qi(i,k)+qitend(i,k)*deltat)/icldm(i,k)   ! +++ Cheng: note it is commented out in our CARMA_cloud. Why?
        dumnc(i,k) = max((nc(i,k)+nctend(i,k)*deltat)/lcldm(i,k),0._r8)
        !dumni(i,k) = max((ni(i,k)+nitend(i,k)*deltat)/icldm(i,k),0._r8)   ! +++ Cheng: commented out in our CARMA_cloud.

        dumr(i,k) = (qr(i,k)+qrtend(i,k)*deltat)/precip_frac(i,k)
        dumnr(i,k) = max((nr(i,k)+nrtend(i,k)*deltat)/precip_frac(i,k),0._r8)
        !dums(i,k) = (qs(i,k)+qstend(i,k)*deltat)/precip_frac(i,k)   ! +++ Cheng: commented out in our CARMA_cloud.
        !dumns(i,k) = max((ns(i,k)+nstend(i,k)*deltat)/precip_frac(i,k),0._r8)   ! +++ Cheng: commented out in our CARMA_cloud.

        dumg(i,k) = (qg(i,k)+qgtend(i,k)*deltat)/precip_frac(i,k)    ! +++ Cheng: Added in MG3
        dumng(i,k) = max((ng(i,k)+ngtend(i,k)*deltat)/precip_frac(i,k),0._r8)    ! +++ Cheng: Added in MG3
  ...
  do k=1,nlev
     ...
     !+++ --- Added by Chuck and Cheng. 2024/4/4. For CARMAcloud DEBUG. 
     !+++ To fix the floating error once turning on the DEBUG=TRUE for CARMAcloud.
     if (do_cldice) then
  ...
  do k=1,nlev
     do i=1,mgncol
        ...
        !-------------------------------------------------------------------
        if ( do_cldliq .and. dumc(i,k) .ge. QSmall ) then  ! JAS 2020 Feb 25
           ...
           ! JAS 2020 Aug 21: vtrmc has already been set to zero
           fc(i,k) = 0._r8
           fnc(i,k)= 0._r8
        endIf  ! JAS 2020 Feb 25

        ! calculate number and mass weighted fall velocity for cloud ice

        if ( do_cldice .and. dumi(i,k) .ge. QSmall ) then  ! JAS 2020 Feb 25
           ...
           ! JAS 2020 Aug 21: vtrmi has already been set to zero
           fi(i,k) = 0._r8
           fni(i,k)= 0._r8
         endIf  ! JAS 2020 Feb 25
  ...
  If ( do_cldliq ) Then  ! JAS 2020 Feb 25 Rain and Snow
   ...
   do k=1,nlev
     ...
     do i=1,mgncol
        if ( Do_CldLiq .and. lamr (i,k) .ge. QSmall ) then  ! JAS 2020 Aug 21
  ...
  EndIf  ! JAS 2020 Feb 25 Rain and Snow
  ...
  do i=1,mgncol
   ...
   If ( do_cldice ) Then  ! JAS 2020 Feb 25 Ice
     ...
     do n = 1,nstep
        ...
        if (do_cldice) then   ! +++ Cheng: JAS deleted it, but we may just keep it to consist with MG version.
           falouti  = fi(i,:)  * dumi(i,:)
   ...
   EndIf  ! JAS 2020 Feb 25 Ice

   !--

   If ( do_cldliq ) Then  ! JAS 2020 Feb 25 CldLiq and Rain
     ...
     do n = 1,nstep
        ...
        !If ( do_cldliq ) Then   ! +++ Cheng: Commente out JAS's if condition. Already in do_cldliq.
   ...
   EndIf  ! JAS 2020 Feb 25 CldLiq and Rain

   !--

   If ( do_cldice ) Then  ! JAS 2020 Feb 25 Snow
   ...
   !EndIf  ! JAS 2020 Feb 25 Snow   ! Cheng: put it in the bottom after do nstep for Graupel in MG3.
  ...
  If ( do_cldice .and. do_cldliq ) Then  ! JAS 2020 Feb 25 Inst Melt Freez
  ...
  enddo
   ...
   !if (do_cldice) then      ! +++ Cheng: commented out by JAS.
      do k=1,nlev
        do i=1,mgncol
   ...
   !-- 
     ...
     do k=1,nlev
        ...
        enddo  ! i 
     enddo   ! k
   !endif      ! +++ Cheng: commented out by JAS.
  EndIf  ! JAS 2020 Feb 25 Inst Melt Freez
  ...
  do k=1,nlev
     do i=1,mgncol
        if (dumc(i,k).ge.qsmall) then
           ...
           !effc(i,k) = 10._r8   ! Commented out by JAS.
           !lamcrad(i,k)=0._r8
           !pgamrad(i,k)=0._r8
           !effc_fn(i,k) = 10._r8
  ...
  If ( do_cldliq ) Then  ! JAS 2020 Feb 25 Rain
  ...
  EndIf  ! do_cldliq JAS 2020 Feb 25 Rain
  ...
  !Endif ! do_cldice JAS 2020 Feb 25 Snow    ! +++ Cheng: add endif after Graupel in MG3.
  ...
  Endif ! do_cldice JAS 2020 Feb 25 Snow
  ...
  do k=1,nlev
     do i=1,mgncol
        ...
        !=================================================================================
        if (do_cldliq .and. qc(i,k)+qctend(i,k)*deltat.lt.qsmall) nctend(i,k)=-nc(i,k)/deltat
        if (do_cldice .and. do_cldice .and. qi(i,k)+qitend(i,k)*deltat.lt.qsmall) nitend(i,k)=-ni(i,k)/deltat
        if (do_cldliq .and. qr(i,k)+qrtend(i,k)*deltat.lt.qsmall) nrtend(i,k)=-nr(i,k)/deltat
        if (do_cldice .and. qs(i,k)+qstend(i,k)*deltat.lt.qsmall) nstend(i,k)=-ns(i,k)/deltat
        !if (qg(i,k)+qgtend(i,k)*deltat.lt.qsmall) ngtend(i,k)=-ng(i,k)/deltat    ! +++ Cheng: added in MG3. 
        if (do_cldice .and. qg(i,k)+qgtend(i,k)*deltat.lt.qsmall) ngtend(i,k)=-ng(i,k)/deltat    ! +++ Cheng: added in MG3. ! do_cldice for now.
     end do
  end do

  ! Zero the tendencies.  Jamison A Smith (JAS) 2020 Aug 21
   ...
   If ( .not. Do_CldIce ) Then
      ...
      qitend = 0._r8
      qstend = 0._r8
      ! +++ Cheng: Zero the graupel like qgtend, ngtend?
   EndIf

   ! End of Zero the Tendencies
