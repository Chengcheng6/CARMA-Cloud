...
module carma_types_mod
  ...
  type, public :: carmastate_type
    ...
    !  qsatfac     Incloud scaling factor for supersaturation []
    !  prain       grid-box averaged rate of conversion of condensate to precip [g/g/s]
    !  nevapr      grid-box averaged rate of evaporation of falling precip [g/g/s]
    !
    ...
    real(kind=f), allocatable, dimension(:)     :: f_qsatfac    ! (NZ)
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
! LW (08/2023): Add for precip rate and evaporation rate of precip
    real(kind=f), allocatable, dimension(:)     :: f_prain      ! (NZ)
    real(kind=f), allocatable, dimension(:)     :: f_nevapr     ! (NZ)
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    !   actpe       Externally specified activation rate. 
    ...
    real(kind=f), allocatable, dimension(:,:,:)   :: f_actpe      ! (NZ,NBIN,NELEM)
    real(kind=f), allocatable, dimension(:)       :: f_clubb_kvh  ! (NZ)
    real(kind=f), allocatable, dimension(:)       :: f_immersebccb
    real(kind=f), allocatable, dimension(:)       :: f_immersed1cb
    real(kind=f), allocatable, dimension(:)       :: f_immersed2cb
    real(kind=f), allocatable, dimension(:)       :: f_immersebcit
    real(kind=f), allocatable, dimension(:)       :: f_immersed1it
    real(kind=f), allocatable, dimension(:)       :: f_immersed2it
    real(kind=f), allocatable, dimension(:)       :: f_awcambc
    real(kind=f), allocatable, dimension(:)       :: f_awcamd1
    real(kind=f), allocatable, dimension(:)       :: f_awcamd2
    real(kind=f), allocatable, dimension(:)       :: f_awfacmbc
    real(kind=f), allocatable, dimension(:)       :: f_awfacmd1
    real(kind=f), allocatable, dimension(:)       :: f_awfacmd2
    real(kind=f), allocatable, dimension(:)       :: f_lcldm
    real(kind=f), allocatable, dimension(:)       :: f_hetrbc
    real(kind=f), allocatable, dimension(:)       :: f_hetrd1
    real(kind=f), allocatable, dimension(:)       :: f_hetrd2
...
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    ! Added Het and Hom nuc tracking variables -> CMALONEY
    real(kind=f), allocatable, dimension(:,:,:)   :: f_pc_so4       ! (NZ,NBIN,NELEM)
    real(kind=f), allocatable, dimension(:,:,:)   :: f_pc_dst       ! The number of aerosols which nucleated into ice
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
