              ...
              if ((iand(inucproc(iepart,ienucto), I_AF_KOOP_2000) /= 0)) then
                ...
                do ibin = 1, NBIN
                  ...
                  if (ssi > sifreeze) then
                    ...
                    !
                    ! NOTE: Got a floating point exception from the power term in
                    ! CONTL when aw was 0., so added a special case for that.
                    if (aw .eq. 0._f) then
                      CONTL =  -2.1133114241_f
                      CONTH =  -1.7620073078_f                    
                    else if (aw < 0.05_f) then
