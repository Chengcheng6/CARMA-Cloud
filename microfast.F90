...
subroutine microfast(carma, cstate, iz, scale_threshold, rc)
  ...
  real(kind=f)                         :: imm_prob
  real(kind=f)                         :: pcliq_old(NBIN)
...
!    call immersenucl(carma,cstate,iz,rc)
!    if (rc < RC_OK) return
...
!    if (rc < RC_OK) return
  ...
  ! zhu 20220919
  ! immersze freezing of water drop
  !imm_prob = min(.99_f,(1._f-exp(-immersebcjimm(iz)*dtime))*immersebcpct(iz)+(1._f-exp(-immersed1jimm(iz)*dtime))*immersed1pct(iz)+(1._f-exp(-immersed2jimm(iz)*dtime))*immersed2pct(iz))
  !pcliq_old = pc(iz,:,2)
  !pc(iz,:,2) = pc(iz,:,2)/(1+imm_prob)
  !pc(iz,:,4) = pc(iz,:,4) + (pcliq_old - pc(iz,:,2))
...
!          if (srat >= ds_threshold(igas)) then
      ...
      else if (ds_threshold(igas) < 0._f)then! .and. t(iz) .lt. 273._f .and. p(iz) .lt. 500000._f) then
