...
!!+++ Cheng: 2025-07-21
!!+++ add eddy diffusion from CLUBB_kvh, and enable debug print_out. 
...
subroutine setupbdif(carma, cstate, rc)
  ...
  real(kind=f)            :: dkz_old, dkz_new   !+++
  !+++
  real(kind=f), allocatable :: clubb_kvh_rev(:)
  real(kind=f), allocatable :: clubb_kvh_mid(:)
  
  !+++
  allocate(clubb_kvh_rev(NZ+1))
  allocate(clubb_kvh_mid(NZ)) 

  !+++
  ! Note: kvh is 1+32 levs but somehow CARMA reload it into clubb_kvh with only 32 lev at the interface.
  !do iz = 1, NZ+1   ! note: can not pass DEBUG
  !  clubb_kvh_rev(iz) = clubb_kvh(NZ+1 - iz + 1)  ! 1+32 lev to 1+32 alt
  !enddo
  do iz = 1, NZ
    !clubb_kvh_rev(iz) = clubb_kvh(NZ - iz + 1)
    clubb_kvh_rev(iz) = clubb_kvh(iz)   ! no reverse upside down.
  end do
  clubb_kvh_rev(NZ+1) = clubb_kvh_rev(NZ)   ! manually set 1+32 lev
  do iz = 1, NZ
    !clubb_kvh_mid(iz) = sqrt( clubb_kvh_rev(iz) * clubb_kvh_rev(iz+1) )  ! geometric mean
    clubb_kvh_mid(iz) = sqrt( max(clubb_kvh_rev(iz), 0._f) * max(clubb_kvh_rev(iz+1), 0._f) )  ! geometric mean with non-negative guard
  enddo
  ...
  5 format('for CLUBB diffusion | iz=',i4,' dkz_old=',1pe11.3,' kvh=',1pe11.3,' dkz_new=',1pe11.3)  !+++
  ...
  do igroup = 1, NGROUP
    ...
    do ibin = 1,NBIN
      ...
      do iz = 1, NZ
        ...
        dkz_old = dkz(iz,ibin,igroup)
        dkz(iz,ibin,igroup) = dkz(iz,ibin,igroup) + clubb_kvh_mid(iz)  
        dkz_new = dkz(iz,ibin,igroup)

#ifdef DEBUG
        if (do_print_init) then
            write(LUNOPRT,5) iz, dkz_old, clubb_kvh_mid(iz), dkz_new     ! print_out (option_1)
        endif
#endif  
  ...
  ! Set bottom boundary before averaging
  dkz(NZP1,:,:) = dkz(NZ,:,:)
...
!+++ to DEBUG
#ifdef DEBUG
  if (do_print_init) write(LUNOPRT,*) 'igridv =', igridv, ',  I_CART =', I_CART
#endif  
  if( igridv .ne. I_CART )then
!+++ to DEBUG
#ifdef DEBUG
    if (do_print_init) write(LUNOPRT,*) 'igridv /= (.ne., not equal) I_CART'
#endif  
    do igroup=1,NGROUP
      do ibin=1,NBIN
...
!            do iz = 1, NZ+1
!                if (do_print_init) then
!                        write(LUNOPRT,*) 'iz = ', iz, ',  zmetl(iz) =', zmetl(iz), ', dkz_scaled_by_zmetl =', dkz(iz,ibin,igroup) 
!                endif 
!            enddo
!        enddo
!    enddo
