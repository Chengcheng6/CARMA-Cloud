...
module carma_flags_mod
  ...
  integer, parameter, public     :: carma_maxdiags = 100
  integer, public                :: carma_ndiagpkgs  ! Number of diags_packages listed
  integer, public                :: carma_ndebugpkgs  ! Number of diags_packages listed
  ...
  logical, public                :: carma_do_budget_diags  = .false.   ! If .true. then do budget diagnostics
  logical, public                :: carma_do_package_diags = .false.   ! If .true. then do package diagnostics
  ...
  integer, public                :: carma_diags_file  = 0         ! Default file for diagnostic output
  ...
  real(r8), public               :: carma_re_size_min_mmr = 1.e-10_r8 ! Smallest mmr for size dependent re calculation (kg/kg)
  real(r8), public               :: carma_re_liquid_size_small = 30._r8  ! Upper limit for small size range of effective radius of liquid (cloud drop) (um) 
  real(r8), public               :: carma_re_liquid_size_large = 50._r8 ! Lower limit for small size range of effective radius of liquid (rain) (um)
  !+++ real(r8), public               :: carma_re_liquid_size_large = 250._r8 ! Lower limit for small size range of effective radius of liquid (rain) (um)
  real(r8), public               :: carma_re_ice_size_small = 100._r8  ! Upper limit for small size range of effective radius of ice (um) 
  real(r8), public               :: carma_re_ice_size_large = 250._r8 ! Lower limit for small size range of effective radius of ice (snow)(um)
  ...
  character(len=12), public      :: carma_diags_packages(carma_maxdiags) = " " ! Names of physics packages for which diagnostic output is desired
  character(len=32), public      :: carma_debug_packages(carma_maxdiags) = " " ! Names of physics packages for which debug output is desired
...
contains
  ...
  subroutine carma_readnl(nlfile)
    ...
    namelist /carma_nl/ &
      ...
      carma_diags_file, &
      carma_re_size_min_mmr, &
      carma_re_liquid_size_small, &
      carma_re_liquid_size_large, &
      carma_re_ice_size_small, &
      carma_re_ice_size_large
    ...
    call mpibcast (carma_do_budget_diags, 1 ,mpilog, 0, mpicom)
    call mpibcast (carma_do_package_diags,1 ,mpilog, 0, mpicom)
    ...
    call mpibcast (carma_diags_file,      1 ,mpiint, 0, mpicom)
    ...
    call mpibcast (carma_re_size_min_mmr, 1 ,mpir8, 0, mpicom)
    call mpibcast (carma_re_liquid_size_small, 1 ,mpir8, 0, mpicom)
    call mpibcast (carma_re_liquid_size_large, 1 ,mpir8, 0, mpicom)
    call mpibcast (carma_re_ice_size_small,    1 ,mpir8, 0, mpicom)
    call mpibcast (carma_re_ice_size_large,    1 ,mpir8, 0, mpicom)
    call mpibcast (carma_model, len(carma_model), mpi_character, 0, mpicom)
