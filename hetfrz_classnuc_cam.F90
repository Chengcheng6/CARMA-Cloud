...
subroutine hetfrz_classnuc_cam_register()
   ...
   ! Zhu 20221021
   call pbuf_add_field('IMBCCB', 'physpkg', dtype_r8, (/pcols,pver/), immbccb_idx)
   call pbuf_add_field('IMD1CB', 'physpkg', dtype_r8, (/pcols,pver/), immd1cb_idx)
   call pbuf_add_field('IMD2CB', 'physpkg', dtype_r8, (/pcols,pver/), immd2cb_idx)
   call pbuf_add_field('IMBCIT', 'physpkg', dtype_r8, (/pcols,pver/), immbcit_idx)
   call pbuf_add_field('IMD1IT', 'physpkg', dtype_r8, (/pcols,pver/), immd1it_idx)
   call pbuf_add_field('IMD2IT', 'physpkg', dtype_r8, (/pcols,pver/), immd2it_idx)
   call pbuf_add_field('AWCAMBC', 'physpkg', dtype_r8, (/pcols,pver/), awcambc_idx)
   call pbuf_add_field('AWCAMD1', 'physpkg', dtype_r8, (/pcols,pver/), awcamd1_idx)
   call pbuf_add_field('AWCAMD2', 'physpkg', dtype_r8, (/pcols,pver/), awcamd2_idx)
   call pbuf_add_field('AWFACMBC', 'physpkg', dtype_r8, (/pcols,pver/), awfacmbc_idx)
   call pbuf_add_field('AWFACMD1', 'physpkg', dtype_r8, (/pcols,pver/), awfacmd1_idx)
   call pbuf_add_field('AWFACMD2', 'physpkg', dtype_r8, (/pcols,pver/), awfacmd2_idx)
   call pbuf_add_field('HETRBC', 'physpkg', dtype_r8, (/pcols,pver/), hetrbc_idx)
   call pbuf_add_field('HETRD1', 'physpkg', dtype_r8, (/pcols,pver/), hetrd1_idx)
   call pbuf_add_field('HETRD2', 'physpkg', dtype_r8, (/pcols,pver/), hetrd2_idx)
   call pbuf_add_field('LCLDM', 'physpkg', dtype_r8, (/pcols,pver/), lcldmbf_idx)
...
subroutine hetfrz_classnuc_cam_calc( &
   ...
   real(r8) :: frzbcimm_jimm(pcols,pver), frzduimm_jimm1(pcols,pver), frzduimm_jimm2(pcols,pver)
   real(r8) :: frzbccnt_jimm(pcols,pver), frzducnt_jimm1(pcols,pver), frzducnt_jimm2(pcols,pver)
   real(r8) :: frzbcimm_pct(pcols,pver), frzduimm_pct1(pcols,pver), frzduimm_pct2(pcols,pver)
   real(r8) :: frzbccnt_pct(pcols,pver), frzducnt_pct1(pcols,pver), frzducnt_pct2(pcols,pver)
      ...
      do k = top_lev, pver
         ...
         if (t(i,k) > 235.15_r8 .and. t(i,k) < 269.15_r8) then
            ...
            call hetfrz_classnuc_calc( &
               ...
               frzbccnt(i,k),  frzducnt(i,k),  frzbcdep(i,k),  frzdudep(i,k),  &
               frzbcimm_jimm(i,k), frzduimm_jimm1(i,k), frzduimm_jimm2(i,k),   &
               frzbccnt_jimm(i,k), frzducnt_jimm1(i,k), frzducnt_jimm2(i,k),   &
               frzbcimm_pct(i,k), frzduimm_pct1(i,k), frzduimm_pct2(i,k),      &
               frzbccnt_pct(i,k), frzducnt_pct1(i,k), frzducnt_pct2(i,k), &
               hetraer(i,k,:), &
...
subroutine hetfrz_classnuc_cam_cbaero( &
   state, deltatin, factnum, pbuf)
   ...
   !zhu 20220919
   real(r8), pointer :: immbccb(:,:)
   real(r8), pointer :: immd1cb(:,:)
   real(r8), pointer :: immd2cb(:,:)
   real(r8), pointer :: immbcit(:,:)
   real(r8), pointer :: immd1it(:,:)
   real(r8), pointer :: immd2it(:,:)   
   real(r8), pointer :: awcambc(:,:)
   real(r8), pointer :: awcamd1(:,:)
   real(r8), pointer :: awcamd2(:,:) 
   real(r8), pointer :: awfacmbc(:,:)
   real(r8), pointer :: awfacmd1(:,:)
   real(r8), pointer :: awfacmd2(:,:)
   real(r8), pointer :: hetrbc(:,:)
   real(r8), pointer :: hetrd1(:,:)
   real(r8), pointer :: hetrd2(:,:)
   real(r8), pointer :: lcldmbf(:,:)
   ...
   character(128) :: errstring   ! Error status
   !-------------------------------------------------------------------------------
   !write(*,*) "hetfrz_classnuc_cam_cbaero start"
   ...
   itim_old = pbuf_old_tim_idx()
   call pbuf_get_field(pbuf, ast_idx, ast, start=(/1,1,itim_old/), kount=(/pcols,pver,1/))
   !write(*,*) "hetfrz_classnuc_cam_cbaero ast",ast
   rho(:,:) = 0._r8
   ...
   do k = top_lev, pver
      do i = 1, ncol
         lcldm(i,k) = max(ast(i,k), mincld)
         !write(*,*) "hetfrz_classnuc_cam_cbaero ast",ast(i,k)
      end do
   end do
   ...
   ! zhu 20220919 
   call pbuf_get_field(pbuf, immbccb_idx, immbccb)
   call pbuf_get_field(pbuf, immd1cb_idx, immd1cb)
   call pbuf_get_field(pbuf, immd2cb_idx, immd2cb)
   call pbuf_get_field(pbuf, immbcit_idx, immbcit)
   call pbuf_get_field(pbuf, immd1it_idx, immd1it)
   call pbuf_get_field(pbuf, immd2it_idx, immd2it)
   call pbuf_get_field(pbuf, awcambc_idx, awcambc)
   call pbuf_get_field(pbuf, awcamd1_idx, awcamd1)
   call pbuf_get_field(pbuf, awcamd2_idx, awcamd2)
   call pbuf_get_field(pbuf, awfacmbc_idx, awfacmbc)
   call pbuf_get_field(pbuf, awfacmd1_idx, awfacmd1)
   call pbuf_get_field(pbuf, awcamd2_idx, awfacmd2)
   call pbuf_get_field(pbuf, hetrbc_idx, hetrbc)
   call pbuf_get_field(pbuf, hetrd1_idx, hetrd1)
   call pbuf_get_field(pbuf, hetrd2_idx, hetrd2)
   call pbuf_get_field(pbuf, lcldmbf_idx,lcldmbf)
   ...
   immbccb(:ncol,:) = total_cloudborne_aer_num(:ncol,:,1)
   immd1cb(:ncol,:) = total_cloudborne_aer_num(:ncol,:,2)
   immd2cb(:ncol,:) = total_cloudborne_aer_num(:ncol,:,3)
   immbcit(:ncol,:) = total_interstitial_aer_num(:ncol,:,1)
   immd1it(:ncol,:) = total_interstitial_aer_num(:ncol,:,2)
   immd2it(:ncol,:) = total_interstitial_aer_num(:ncol,:,3)
   awcambc(:ncol,:) = awcam(:ncol,:,1)
   awcamd1(:ncol,:) = awcam(:ncol,:,2)
   awcamd2(:ncol,:) = awcam(:ncol,:,3)
   awfacmbc(:ncol,:) = awfacm(:ncol,:,1)
   awfacmd1(:ncol,:) = awfacm(:ncol,:,2)
   awfacmd2(:ncol,:) = awfacm(:ncol,:,3)
   hetrbc(:ncol,:) = hetraer(:ncol,:,1)
   hetrd1(:ncol,:) = hetraer(:ncol,:,2)
   hetrd2(:ncol,:) = hetraer(:ncol,:,3)
   lcldmbf(:ncol,:) = lcldm(:ncol,:)
   !write(*,*) "hetfrz_classnuc_cam_cbaero end"

   end associate

end subroutine hetfrz_classnuc_cam_cbaero
