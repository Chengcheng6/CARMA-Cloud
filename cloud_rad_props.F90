...
use rad_constituents, only: iceopticsfile, liqopticsfile, liqcldoptics
...
use carma_flags_mod, only: carma_do_cldice, carma_do_cldliq
...
public :: &
   ...
   get_snow_optics_sw,            &
   get_liquid_optics_sw_carma,    & ! return CARMA liquid clouds SW optical props
   liquid_cloud_get_rad_props_lw_carma  ! return CARMA liquid clouds LW optical props
...
!++++++++++++++++++++ June 2022 LW: CARMA Liq Clouds +++++++++++++++++++++++++
! variables below should be all initialized
integer :: nbin                            ! # of CARMA bins
integer :: cld_idx      = 0                ! cloud fraction index
!integer, allocatable :: ixcldliq_carma(:)        ! cloud liquid water index (should come from namelist)

real(r8), allocatable :: ext_sw_liq_carma(:,:)   !(nbin,nwave)
real(r8), allocatable :: ssa_sw_liq_carma(:,:)
real(r8), allocatable :: asm_sw_liq_carma(:,:)
real(r8), allocatable :: abs_lw_liq_carma(:,:)
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
! 
   ...
   !!-------------- Zhu Sep 12 2024 ------------------
   integer :: i_icgrpwp_carma, i_deg_carma
   !!--------------------------------------------------
...
!++++++++++++++++++++ June 2022 LW: CARMA Liq Clouds +++++++++++++++++++++++++
   ! liquid clouds (CARMA)
   integer :: bindimid ! bin dimension
   integer :: ext_sw_liq_carma_id, ssa_sw_liq_carma_id, asm_sw_liq_carma_id, abs_lw_liq_carma_id

!     carma_get to get the bin carma is using
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
   ...
   !! ------------ Zhu Sep 12 2024 --------------------
   i_icgrpwp_carma = pbuf_get_index('ICGRPCARMA',errcode=err)
   i_deg_carma = pbuf_get_index('DEGCARMA',errcode=err)
   !! ---------------------------------------------------------
!++++++++++++++++++++++++++++++++++++++++++++++++++++
...
! +++++++++++++++++++ June 2022 LW: add in 'carmaliqopt' for CARMA liq clouds +++++++++++++++++++
   case ('carmaliqopt')
   ! read CARMA liquid cloud optics
   if(masterproc) then
   call getfil( trim(liquidfile), locfn, 0)
   call handle_ncerr( nf90_open(locfn, NF90_NOWRITE, ncid), 'liquid optics file missing')
   write(iulog,*)' reading liquid cloud optics from file ',locfn
   ...
   call handle_ncerr(nf90_inq_dimid( ncid, 'nbin_idx', bindimid), 'getting carma bin dim')
   call handle_ncerr(nf90_inquire_dimension( ncid, bindimid, len=nbin), 'getting n carma rbins')

   endif ! if (masterproc)

#if ( defined SPMD )
   call mpibcast(nbin, 1, mpiint, 0, mpicom, ierr)
#endif

!   allocate(ext_sw_liq_carma(nswbands,nbin))
!   allocate(ssa_sw_liq_carma(nswbands,nbin))
!   allocate(asm_sw_liq_carma(nswbands,nbin))
!   allocate(abs_lw_liq_carma(nswbands,nbin))
   allocate(ext_sw_liq_carma(nbin,nswbands) )
   allocate(ssa_sw_liq_carma(nbin,nswbands))
   allocate(asm_sw_liq_carma(nbin,nswbands))
   allocate(abs_lw_liq_carma(nbin,nlwbands))

   if(masterproc) then
   call handle_ncerr( nf90_inq_varid(ncid, 'ext_sw', ext_sw_liq_carma_id),&
      'cloud optics ext_sw_liq get')
   call handle_ncerr( nf90_get_var(ncid, ext_sw_liq_carma_id, ext_sw_liq_carma),&
      'read cloud optics ext_sw_liq values')

   call handle_ncerr( nf90_inq_varid(ncid, 'ssa_sw', ssa_sw_liq_carma_id),&
      'cloud optics ssa_sw_liq get')
   call handle_ncerr( nf90_get_var(ncid, ssa_sw_liq_carma_id, ssa_sw_liq_carma),&
      'read cloud optics ssa_sw_liq values')

   call handle_ncerr( nf90_inq_varid(ncid, 'asm_sw', asm_sw_liq_carma_id),&
      'cloud optics asm_sw_liq get')
   call handle_ncerr( nf90_get_var(ncid, asm_sw_liq_carma_id, asm_sw_liq_carma),&
      'read cloud optics asm_sw_liq values')

   call handle_ncerr( nf90_inq_varid(ncid, 'abs_lw', abs_lw_liq_carma_id),&
      'cloud optics abs_lw_liq get')
   call handle_ncerr( nf90_get_var(ncid, abs_lw_liq_carma_id, abs_lw_liq_carma),&
      'read cloud optics abs_lw_liq values')
...
#if ( defined SPMD )
    call mpibcast(ext_sw_liq_carma, nbin*nswbands, mpir8, 0, mpicom, ierr)
    call mpibcast(ssa_sw_liq_carma, nbin*nswbands, mpir8, 0, mpicom, ierr)
    call mpibcast(asm_sw_liq_carma, nbin*nswbands, mpir8, 0, mpicom, ierr)
    call mpibcast(abs_lw_liq_carma, nbin*nlwbands, mpir8, 0, mpicom, ierr)
#endif
! +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

    case default
        call endrun('liqcldoptics must be either slingo or gammadist or carmaliqopt')
    end select
...
subroutine get_ice_optics_sw(state, pbuf, tau, tau_w, tau_w_g, tau_w_f)
   ...
   !!--------------- Zhu Sep 12 2024 ---------------------
   real(r8), pointer :: icgrpwp_carma(:,:), deg_carma(:,:)

   real(r8) :: tau_carma    (nswbands,pcols,pver)
   real(r8) :: tau_w_carma  (nswbands,pcols,pver)
   real(r8) :: tau_w_g_carma(nswbands,pcols,pver)
   real(r8) :: tau_w_f_carma(nswbands,pcols,pver)
   !!------------------------------------------------------
   ...
   if(carma_do_cldice .and. carma_do_cldliq)then
     call pbuf_get_field(pbuf, i_icgrpwp_carma, icgrpwp_carma)
     call pbuf_get_field(pbuf, i_deg_carma,   deg_carma)
   end if
   !!------------------------------------------------------
   ...
   !!--------------- Zhu Sep 12 2024 ---------------------
   if(carma_do_cldice .and. carma_do_cldliq)then
     call interpolate_ice_optics_sw(state%ncol, icgrpwp_carma, deg_carma, &
                   tau_carma, tau_w_carma, &        
                   tau_w_g_carma, tau_w_f_carma)
   end if

   tau = tau+tau_carma
   tau_w = tau_w + tau_w_carma
   tau_w_g = tau_w_g + tau_w_g_carma
   tau_w_f = tau_w_f + tau_w_f_carma
   !!------------------------------------------------------
...
subroutine interpolate_ice_optics_sw(ncol, iciwpth, dei, tau, tau_w, &
  ...
  do k = 1,pver
     do i = 1,ncol
        ...
        !write(*,*) 'print g_d_eff, n_g_d values:', g_d_eff, n_g_d
...
! ++++++++++++++ June 2022 LW: routine for CARMA liq clouds SW opt props +++++++++++++++++
subroutine get_liquid_optics_sw_carma(state, pbuf, tau, tau_w, tau_w_g, tau_w_f)
   use constituents,        only: cnst_get_ind
   ...
   real(r8) ::  clwpth_carma           ! CARMA liquid water path for each bin
   real(r8) ::  deltap_over_g_over_CF 
   real(r8), pointer :: cld(:,:)       ! cloud fraction
   integer :: itim_old
   integer i,k,j,swband,lchnk,ncol
   integer :: ixcldliq_carma        ! cloud liquid water index (should come from namelist)
   ...
   do k = 1,pver
      do i = 1,ncol
        deltap_over_g_over_CF=state%pdel(i,k)/gravit/max(0.0001_r8,cld(i,k))
         
         if(cld(i,k) > 0.01_r8)then   
         do j = 1,nbin
          write (outputbin, "(I2.2)") j
          call cnst_get_ind('CRLIQ'//outputbin, ixcldliq_carma)
          !write(*,*) "get CRLIQ index ",'CRLIQ'//outputbin
        
          ! state%q(i,k,ixcldliq_carma(j)) should correspond to CRLIQ01,02... in kg/kg, clwpth_carma is in kg/m2
          clwpth_carma = state%q(i,k,ixcldliq_carma) * deltap_over_g_over_CF
          ! 1097 is the item# for CRLIQ01,02... in the MASTER FIELD LIST in the atm.log.xxx file
          ! clwpth_carma = state%q(i,k,1097+(6*(j-1))) * deltap_over_g_over_CF
          !if(cld(i,k) > 0.01_r8)then
          if(clwpth_carma > 1.e-80_r8) then ! This seems to be clue from microphysics of no cloud
!            call carma_liquid_sw(clwpth_carma, j, &
!                tau(j,1:nswbands,i,k), tau_w(j,1:nswbands,i,k), tau_w_g(j,1:nswbands,i,k), tau_w_f(j,1:nswbands,i,k))
!        =================================
            tau(1:nswbands,i,k) = tau (1:nswbands,i,k) + clwpth_carma * ext_sw_liq_carma (j,1:nswbands)
            tau_w(1:nswbands,i,k) = tau_w(1:nswbands,i,k) + clwpth_carma * ext_sw_liq_carma (j,1:nswbands) * ssa_sw_liq_carma(j,1:nswbands)
            tau_w_g(1:nswbands,i,k) = tau_w_g(1:nswbands,i,k) + clwpth_carma * ext_sw_liq_carma (j,1:nswbands) * ssa_sw_liq_carma(j,1:nswbands) * asm_sw_liq_carma(j,1:nswbands)
            tau_w_f(1:nswbands,i,k) = tau_w_f(1:nswbands,i,k) + clwpth_carma * ext_sw_liq_carma (j,1:nswbands) * ssa_sw_liq_carma(j,1:nswbands) * asm_sw_liq_carma(j,1:nswbands) * asm_sw_liq_carma(j,1:nswbands)
!        =================================
          endif  !if(clwpth_carma > 1.e-80_r8) 
        enddo  !do j = 1,nbin
        endif
      enddo  ! do i = 1,ncol
   enddo    !do k = 1,pver

end subroutine get_liquid_optics_sw_carma
! ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
! ++++++++++++++ June 2022 LW: routine for CARMA liq clouds LW opt props +++++++++++++++++
subroutine liquid_cloud_get_rad_props_lw_carma(state, pbuf, abs_od)
   use constituents,        only: cnst_get_ind
   ...
   do k = 1,pver
      do i = 1,ncol
        ...
        do j = 1,nbin
         ...
         !  if(cld(i,k) > 0.01_r8)then
           if(clwpth_carma > 1.e-80_r8) then ! This seems to be clue from microphysics of no cloud
             abs_od(1:nlwbands,i,k) = abs_od(1:nlwbands,i,k) + clwpth_carma * abs_lw_liq_carma(j,1:nlwbands)
           !else
           !   abs_od(1:nlwbands,i,k) = abs_od(1:nlwbands,i,k) + 0._r8
           endif  !if(clwpth_carma > 1.e-80_r8)
         enddo  !j = 1,nbin
         endif
      enddo  !do i = 1,ncol
...
end subroutine liquid_cloud_get_rad_props_lw_carma
! ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
subroutine ice_cloud_get_rad_props_lw(state, pbuf, abs_od)
   ...
   !!--------------- Zhu Dec 09 2024 ---------------------
   real(r8), pointer :: icgrpwp_carma(:,:), deg_carma(:,:)
   real(r8) :: abs_od_carma    (nlwbands,pcols,pver)
   !!------------------------------------------------------
   ...
   !!--------------- Zhu Dec 09 2024 ---------------------
   if(carma_do_cldice .and. carma_do_cldliq)then
     call interpolate_ice_optics_lw(state%ncol, icgrpwp_carma, deg_carma, abs_od_carma)
   end if

   abs_od = abs_od+abs_od_carma
   !!------------------------------------------------------

end subroutine ice_cloud_get_rad_props_lw

!+++ Noted by Cheng 20241120: Seems that we are missing CARMA graupel to LWCF?
!+++ 20241209 Yes. Zhu added graupel LWCF above.
...
!==============================================================================
!+++++++++++++++ routine carma_liquid_sw() may be unnecessary +++++++++++++++++
!==============================================================================

subroutine carma_liquid_sw(clwpth_carma, ibin, tau, tau_w, tau_w_g, tau_w_f)
  real(r8), intent(in) :: clwpth_carma ! cloud water liquid path new (in cloud) (in g/m^2)?
  integer, intent(in) :: ibin
  !real(r8), intent(inout) :: tau(1:nswbands), tau_w(1:nswbands), tau_w_f(1:nswbands), tau_w_g(1:nswbands)
  real(r8), intent(out) :: tau(1:nswbands), tau_w(1:nswbands), tau_w_f(1:nswbands), tau_w_g(1:nswbands)

  !real(r8) :: ext_carma(nbin,nswbands), ssa_carma(nbin,nswbands), asm_carma(nbin,nswbands)

! Read in the table for optical properties from CARMA Mie calculation
  

  ! compute radiative properties
  !tau(:) = tau (:) + clwptn * ext_sw_liq_carma (ibin,:)
  !tau_w(:) = tau_w(:) + clwptn * ext_sw_liq_carma (ibin,:) * ssa_sw_liq_carma(ibin,:)
  !tau_w_g(:) = tau_w_g(:) + clwptn * ext_sw_liq_carma (ibin,:) * ssa_sw_liq_carma(ibin,:) * asm_sw_liq_carma(ibin,:)
  !tau_w_f(:) = tau_w_f(:) + clwptn * ext_sw_liq_carma (ibin,:) * ssa_sw_liq_carma(ibin,:) * asm_sw_liq_carma(ibin,:) * asm_sw_liq_carma(ibin,:)
  tau(:) = clwpth_carma * ext_sw_liq_carma (ibin,:)
  tau_w(:) = clwpth_carma * ext_sw_liq_carma (ibin,:) * ssa_sw_liq_carma(ibin,:)
  tau_w_g(:) = clwpth_carma * ext_sw_liq_carma (ibin,:) * ssa_sw_liq_carma(ibin,:) * asm_sw_liq_carma(ibin,:)
  tau_w_f(:) = clwpth_carma * ext_sw_liq_carma (ibin,:) * ssa_sw_liq_carma(ibin,:) * asm_sw_liq_carma(ibin,:) * asm_sw_liq_carma(ibin,:)
end subroutine carma_liquid_sw
