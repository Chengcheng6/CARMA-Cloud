...
!if ( rnuclg(ifrom,igfrom,igroup) .gt. 0._f) then
!write(LUNOPRT,*) ' '
!write(LUNOPRT,*) ' ifrom =', ifrom
!write(LUNOPRT,*) ' ibin =', ibin
!write(LUNOPRT,*) ' iefrom =', iefrom
!write(LUNOPRT,*) ' ielem =', ielem
!write(LUNOPRT,*) ' igfrom =', igfrom
!write(LUNOPRT,*) ' igroup =', igroup
!write(LUNOPRT,*) ' rnuclg =', rnuclg(ifrom,igfrom,igroup)
!end if

               !!!!!! Parse out the heterogenous nucleation from freezing on dust (homogeneous freezing of pure water (sufluric acid) is handled in upgxfer.F90) <--- CMALONEY
                pc_dst(iz,ibin,ielem) = pc_dst(iz,ibin,ielem) + rnucprod_nd
               !!!!!!!
