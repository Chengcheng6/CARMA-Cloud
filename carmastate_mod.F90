  ...
  subroutine CARMASTATE_Allocate(cstate, rc)
    ...
    if (.not. (allocated(cstate%f_xmet))) then
      ...
      allocate( &
        ...
        cstate%f_cldfrc(NZ), &
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
! LW (08/2023): Add for determining precip rate and evaporation rate of precip
        cstate%f_prain(NZ), &
        cstate%f_nevapr(NZ), &      
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++  
        cstate%f_qsatfac(NZ), &
        ...
        cstate%f_actpe(NZ,NBIN,NELEM), &
        cstate%f_clubb_kvh(NZ), &
        cstate%f_immersebccb(NZ), &
        cstate%f_immersed1cb(NZ), &
        cstate%f_immersed2cb(NZ), &
        cstate%f_immersebcit(NZ), &
        cstate%f_immersed1it(NZ), &
        cstate%f_immersed2it(NZ), &
        cstate%f_awcambc(NZ), &
        cstate%f_awcamd1(NZ), &
        cstate%f_awcamd2(NZ), &
        cstate%f_awfacmbc(NZ), &
        cstate%f_awfacmd1(NZ), &
        cstate%f_awfacmd2(NZ), &
        cstate%f_lcldm(NZ), &
        cstate%f_hetrbc(NZ), &
        cstate%f_hetrd1(NZ), &
        cstate%f_hetrd2(NZ), &
...
! LW (08/2023): Add for determining precip rate and evaporation rate of precip      
      cstate%f_prain(:)      = 0._f
      cstate%f_nevapr(:)      = 0._f
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++  
      cstate%f_qsatfac(:)   = 1._f

      cstate%f_immersebccb(:)       = 0._f
      cstate%f_immersed1cb(:)       = 0._f
      cstate%f_immersed2cb(:)       = 0._f
      cstate%f_immersebcit(:)       = 0._f
      cstate%f_immersed1it(:)       = 0._f
      cstate%f_immersed2it(:)       = 0._f
      cstate%f_awcambc(:)           = 0._f
      cstate%f_awcamd1(:)           = 0._f
      cstate%f_awcamd2(:)           = 0._f
      cstate%f_awfacmbc(:)          = 0._f
      cstate%f_awfacmd1(:)          = 0._f
      cstate%f_awfacmd2(:)          = 0._f
      cstate%f_lcldm(:)             = 0._f
      cstate%f_hetrbc(:)            = 0._f
      cstate%f_hetrd1(:)            = 0._f
      cstate%f_hetrd2(:)            = 0._f
...
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ! LW (Sep 2022): added for homo. & hetero. nuc.
          cstate%f_pc_so4(NZ,NBIN,NELEM), &
          cstate%f_pc_dst(NZ,NBIN,NELEM), &
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
          stat=ier)
        if (ier /= 0) then
          if (cstate%f_carma%f_do_print) then
...
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++  
        ...
        cstate%f_actpe, &
        cstate%f_clubb_kvh, &
        cstate%f_immersebccb, &
        cstate%f_immersed1cb, &
        cstate%f_immersed2cb, &
        cstate%f_immersebcit, &
        cstate%f_immersed1it, &
        cstate%f_immersed2it, &
        cstate%f_awcambc, &
        cstate%f_awcamd1, &
        cstate%f_awcamd2, &
        cstate%f_awfacmbc, &
...
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  !! @author Chuck Bardeen
  !! @version Feb-2009
  subroutine CARMASTATE_Step(cstate, rc, cldfrc, qsatfac, lndfv, ocnfv, icefv, lndram, ocnram, iceram, lndfrac, ocnfrac, icefrac)
    type(carmastate_type), intent(inout)  :: cstate
    integer, intent(out)                  :: rc
    real(kind=f), intent(in), optional    :: cldfrc(cstate%f_NZ)  !! cloud fraction [fraction]
    real(kind=f), intent(in), optional    :: qsatfac(cstate%f_NZ) !! subgrid supersaturation factor []
    ...
    ! Store the cloud fraction and supgrid saturation factor if specified
    cstate%f_cldfrc(:) = 1._f
    cstate%f_qsatfac(:) = 1._f
    
    if (present(cldfrc)) cstate%f_cldfrc(:)=cldfrc(:)
    if (present(qsatfac)) cstate%f_qsatfac(:)=qsatfac(:)
  ...
  !subroutine CARMASTATE_Get(cstate, rc, max_nsubstep, max_nretry, nstep, nsubstep, nretry, zsubsteps, lat, lon, cldfrc)
  subroutine CARMASTATE_Get(cstate, rc, max_nsubstep, max_nretry, nstep, nsubstep, nretry, zsubsteps, lat, lon, cldfrc, prain, nevapr)
  ! (LW 08/2023: 'prain' and 'nevapr' are added for precip and evap rate)
    ...
    real(kind=f), optional, intent(out)   :: cldfrc(cstate%f_NZ) !! cloud fraction [fraction]
    real(kind=f), optional, intent(out)   :: prain(cstate%f_NZ)  !! grid-box averaged rate of conversion of condensate to precip [g/g/s] (LW 08/2023)
    real(kind=f), optional, intent(out)   :: nevapr(cstate%f_NZ) !! grid-box averaged rate of evaporation of falling precip [g/g/s] (LW 08/2023)
    ...
    if (present(cldfrc))       cldfrc       = cstate%f_cldfrc
    if (present(prain))        prain        = cstate%f_prain    !(LW 08/2023): [g/g/s]
    if (present(nevapr))       nevapr       = cstate%f_nevapr   !(LW 08/2023): [g/g/s]
  ...
  subroutine CARMASTATE_GetBin(cstate, ielem, ibin, mmr, rc, &
                               ...
                               surface, sedimentationflux, vf, vd, dtpart, so4Nucleated, dstNucleated)
    type(carmastate_type), intent(in)     :: cstate         !! the carma state object
...
!    real(kind=f), optional, intent(out)   :: homjn(cstate%f_NZ)
!    real(kind=f), optional, intent(out)   :: hetjn(cstate%f_NZ) 
    real(kind=f), optional, intent(out)   :: so4Nucleated(cstate%f_NZ)
    real(kind=f), optional, intent(out)   :: dstNucleated(cstate%f_NZ)
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ! LW (Sep 2022): added for hetero. nuc.
    !!!!!!!!CMALONEY: I think this goes here, track the het/hom nuc rates
        if (present(so4Nucleated)) so4Nucleated(:)  = cstate%f_pc_so4(:, ibin, ielem) / (cstate%f_xmet(:)*cstate%f_ymet(:)*cstate%f_zmet(:))
        if (present(dstNucleated)) dstNucleated(:) = cstate%f_pc_dst(:, ibin, ielem) / (cstate%f_xmet(:)*cstate%f_ymet(:)*cstate%f_zmet(:))
    !!!!!!!!
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  subroutine CARMASTATE_GetGas(cstate, igas, mmr, rc, satice, satliq, eqice, eqliq, wtpct, &
     immbccb_out,immd1cb_out,immd2cb_out,immbcit_out,immd1it_out,immd2it_out, &
     awcambc_out,awcamd1_out,awcamd2_out,awfacmbc_out,awfacmd1_out,awfacmd2_out,lcldm_out, &
     hetrbc_out,hetrd1_out,hetrd2_out)
    type(carmastate_type), intent(in)     :: cstate            !! the carma state object
    ...
    real(kind=f), optional, intent(out)    :: immbccb_out(cstate%f_NZ)
    real(kind=f), optional, intent(out)    :: immd1cb_out(cstate%f_NZ)
    real(kind=f), optional, intent(out)    :: immd2cb_out(cstate%f_NZ)
    real(kind=f), optional, intent(out)    :: immbcit_out(cstate%f_NZ)
    real(kind=f), optional, intent(out)    :: immd1it_out(cstate%f_NZ)
    real(kind=f), optional, intent(out)    :: immd2it_out(cstate%f_NZ)
    real(kind=f), optional, intent(out)    :: awcambc_out(cstate%f_NZ)
    real(kind=f), optional, intent(out)    :: awcamd1_out(cstate%f_NZ)
    real(kind=f), optional, intent(out)    :: awcamd2_out(cstate%f_NZ)
    real(kind=f), optional, intent(out)    :: awfacmbc_out(cstate%f_NZ)
    real(kind=f), optional, intent(out)    :: awfacmd1_out(cstate%f_NZ)
    real(kind=f), optional, intent(out)    :: awfacmd2_out(cstate%f_NZ)
    real(kind=f), optional, intent(out)    :: lcldm_out(cstate%f_NZ)
    real(kind=f), optional, intent(out)    :: hetrbc_out(cstate%f_NZ)
    real(kind=f), optional, intent(out)    :: hetrd1_out(cstate%f_NZ)
    real(kind=f), optional, intent(out)    :: hetrd2_out(cstate%f_NZ)
    ...
    !write(*,*) "CARMASTATE_GetGas before out" 
    !zhu 20220930
    if (present(immbccb_out)) immbccb_out(:) = cstate%f_immersebccb(:)
    if (present(immd1cb_out)) immd1cb_out(:) = cstate%f_immersed1cb(:)
    if (present(immd2cb_out)) immd2cb_out(:) = cstate%f_immersed2cb(:)
    !write(*,*) "immd2cb_out"
    if (present(immbcit_out)) immbcit_out(:) = cstate%f_immersebcit(:)
    if (present(immd1it_out)) immd1it_out(:) = cstate%f_immersed1it(:)
    if (present(immd2it_out)) immd2it_out(:) = cstate%f_immersed2it(:)
    !write(*,*) "immd2it_out"
    if (present(awcambc_out)) awcambc_out(:) = cstate%f_awcambc(:)
    if (present(awcamd1_out)) awcamd1_out(:) = cstate%f_awcamd1(:)
    if (present(awcamd2_out)) awcamd2_out(:) = cstate%f_awcamd2(:)
    !write(*,*) "awcamd2_out"
    if (present(awfacmbc_out)) awfacmbc_out(:) = cstate%f_awfacmbc(:)
    if (present(awfacmd1_out)) awfacmd1_out(:) = cstate%f_awfacmd1(:)
    if (present(awfacmd2_out)) awfacmd2_out(:) = cstate%f_awfacmd2(:)
    !write(*,*) "awfacmd2_out"
    !write(*,*) "CARMASTATE_GetGas cstate%f_lcldm(:)",cstate%f_lcldm(:)
    if (present(lcldm_out)) lcldm_out(:) = cstate%f_lcldm(:)
    !write(*,*) "lcldm_out"
    if (present(hetrbc_out)) hetrbc_out(:) = cstate%f_hetrbc(:)
    if (present(hetrd1_out)) hetrd1_out(:) = cstate%f_hetrd1(:)
    if (present(hetrd2_out)) hetrd2_out(:) = cstate%f_hetrd2(:)
    !write(*,*) "hetrd2_out"
  ...
  subroutine CARMASTATE_SetBin(cstate, ielem, ibin, mmr, rc, surface, activationrate, clubb_kvh)
    type(carmastate_type), intent(inout)  :: cstate         !! the carma state object
    integer, intent(in)                   :: ielem          !! the element index
    integer, intent(in)                   :: ibin           !! the bin index
    ...
    real(kind=f), optional, intent(in)    :: activationrate(cstate%f_NZ) !! droplet activation rate [#/kg/s]
    real(kind=f), optional, intent(in)    :: clubb_kvh(cstate%f_NZ) !!eddy diffusivity m2/s
    integer                               :: igroup         ! Group containing this bin
    ...
    if (present(activationrate))    cstate%f_actpe(:,ibin,ielem) = activationrate(:)*1.e-3_f* cstate%f_rhoa_wet(:)   !Convert activationrate to g/x/y/z/s
    if (present(clubb_kvh)) cstate%f_clubb_kvh(:) = clubb_kvh(:)
  ...
  !! @see CARMA_Step 
  subroutine CARMASTATE_SetGas(cstate, igas, mmr, rc, mmr_old, satice_old, satliq_old, immbccb,immd1cb,immd2cb,immbcit,immd1it,immd2it,awcambc,awcamd1,awcamd2,awfacmbc,awfacmd1,awfacmd2,lcldm,hetrbc,hetrd1,hetrd2)
    ...
    real(kind=f), optional, intent(in)    :: immbccb(cstate%f_NZ)
    real(kind=f), optional, intent(in)    :: immd1cb(cstate%f_NZ)
    real(kind=f), optional, intent(in)    :: immd2cb(cstate%f_NZ)
    real(kind=f), optional, intent(in)    :: immbcit(cstate%f_NZ)
    real(kind=f), optional, intent(in)    :: immd1it(cstate%f_NZ)
    real(kind=f), optional, intent(in)    :: immd2it(cstate%f_NZ)
    real(kind=f), optional, intent(in)    :: awcambc(cstate%f_NZ)
    real(kind=f), optional, intent(in)    :: awcamd1(cstate%f_NZ)
    real(kind=f), optional, intent(in)    :: awcamd2(cstate%f_NZ)
    real(kind=f), optional, intent(in)    :: awfacmbc(cstate%f_NZ)
    real(kind=f), optional, intent(in)    :: awfacmd1(cstate%f_NZ)
    real(kind=f), optional, intent(in)    :: awfacmd2(cstate%f_NZ)
    real(kind=f), optional, intent(in)    :: lcldm(cstate%f_NZ)
    real(kind=f), optional, intent(in)    :: hetrbc(cstate%f_NZ)
    real(kind=f), optional, intent(in)    :: hetrd1(cstate%f_NZ)
    real(kind=f), optional, intent(in)    :: hetrd2(cstate%f_NZ)
    ...
    !zhu 20220919
    if (present(immbccb)) cstate%f_immersebccb(:) = immbccb(:)
    if (present(immd1cb)) cstate%f_immersed1cb(:) = immd1cb(:)
    if (present(immd2cb)) cstate%f_immersed2cb(:) = immd2cb(:)
    if (present(immbcit)) cstate%f_immersebcit(:) = immbcit(:)
    if (present(immd1it)) cstate%f_immersed1it(:) = immd1it(:)
    if (present(immd2it)) cstate%f_immersed2it(:) = immd2it(:)
    if (present(awcambc)) cstate%f_awcambc(:) = awcambc(:)
    if (present(awcamd1)) cstate%f_awcamd1(:) = awcamd1(:)
    if (present(awcamd2)) cstate%f_awcamd2(:) = awcamd2(:)
    if (present(awfacmbc)) cstate%f_awfacmbc(:) = awfacmbc(:)
    if (present(awfacmd1)) cstate%f_awfacmd1(:) = awfacmd1(:)
    if (present(awfacmd2)) cstate%f_awfacmd2(:) = awfacmd2(:)
    if (present(lcldm)) cstate%f_lcldm(:) = lcldm(:)
    if (present(hetrbc)) cstate%f_hetrbc(:) = hetrbc(:)
    if (present(hetrd1)) cstate%f_hetrd1(:) = hetrd1(:)
    if (present(hetrd2)) cstate%f_hetrd2(:) = hetrd2(:)
