...
logical,  intent(in) :: do_cldliq             ! JAS 2020 Feb 24 Prognosing cldliq  
...
real(r8), intent(in) :: re_liq(:,:)    ! JAS 2020 Feb 24, liquid effective radius (m)
...
!!! recalculate effective radius for constant number, in order to separate
      ...
      ! Note: If Carma is doing the liquid microphysics, then the effective
      ! radius is...

      If ( .not. do_cldliq ) Then
         effc (i,k) = re_liq (i,k) * 1e6_r8      ! m -> um
      EndIf
...
500 continue
   ...
   do k=top_lev,pver
      ...
      if ( do_cldliq .and. qc (i,k) + qctend (i,k) * deltat .lt. qsmall ) &
       nctend (i,k) = -nc (i,k) / deltat
      if ( do_cldice .and. qi (i,k) + qitend (i,k) * deltat .lt. qsmall ) &
