...
! ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
! +++ Commented by Cheng. 3 new variables found in the CARMA cloud code.
   real(r8) :: liq_icld_vistau_3451(pcols,pver) ! in-cld liq cloud optical depth (3.5µm, only during day, night = fillvalue)
   real(r8) :: liq_icld_visssa_3451(pcols,pver) ! in-cld liq single scattering albedo (3.5µm, during day, night = fillvalue)
   real(r8) :: liq_icld_lwabs_3451(pcols,pver)  ! in-cld liq LW absorption (3.5µm)
! ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
subroutine radiation_init(pbuf2d)
   ...
   call addfld('ICE_ICLD_VISTAU', (/ 'lev' /), 'A',  '1', 'Ice in-cloud extinction visible sw optical depth', &
                                                       sampling_seq='rad_lwsw', flag_xyfill=.true.)
! ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++                                                      
   call addfld('LIQ_ICLD_VISTAU3451', (/ 'lev' /), 'A',  '1', 'Liquid in-cloud extinction visible sw optical depth, 3.5µm', &
                                                       sampling_seq='rad_lwsw', flag_xyfill=.true.)
   call addfld('LIQ_ICLD_VISSSA3451', (/ 'lev' /), 'A',  '1', 'Liquid in-cloud ssa visible sw, 3.5µm', &
                                                       sampling_seq='rad_lwsw', flag_xyfill=.true.)
   call addfld('LIQ_ICLD_LWABS3451', (/ 'lev' /), 'A',  '1', 'Liquid in-cloud extinction visible lw optical depth, 3.5µm', &
                                                       sampling_seq='rad_lwsw', flag_xyfill=.true.)
! ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
subroutine radiation_tend( &
   ...
   ! 2022-06-07  L. Wang          Modify to use CARMA cloud optical properties with rrtmg.
   ! +++ ! 2023-02-22  C.C. Liu   Upgraded CARMA cloud from CESM211 to CESM222 base code.    
   !--------------------------------------------------------------------------------------
   ...
   use cloud_rad_props,    only: get_ice_optics_sw, get_liquid_optics_sw, liquid_cloud_get_rad_props_lw, &
                                 ...
                                 get_liquid_optics_sw_carma, liquid_cloud_get_rad_props_lw_carma
   ...
   ! Commented by Cheng: no pointer anymore in CESM2.2.2
   ...
   if (dosw .or. dolw) then
      ...
      ! Commented by Cheng: no pointer anymore in CESM2.2.2. Bug occurs if using pointer.
...
! ++++++++++++++++++++++++++June 2022 LW: add in 'carmaliqopt' for CARMA clouds ++++++++++++++++++++++++++
            case ('carmaliqopt')
               call get_liquid_optics_sw_carma(state, pbuf, liq_tau, liq_tau_w, liq_tau_w_g, liq_tau_w_f)
! ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            case default
               call endrun('liqcldoptics must be either slingo or gammadist or carmaliqopt(for carma cloud)')
            end select
         end if
...
! ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
         ! Output cloud optical depth fields for the visible band (3.5 µm)
         rd%liq_icld_vistau_3451(:ncol,:) = liq_tau(1,:ncol,:)
         rd%liq_icld_visssa_3451(:ncol,:) = liq_tau_w(1,:ncol,:)
! ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
! ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            rd%liq_icld_vistau_3451(IdxNite(i),:) = fillvalue
            rd%liq_icld_visssa_3451(IdxNite(i),:) = fillvalue
! ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            if (cldfsnow_idx > 0) then
...
! ++++++++++++++++++++++++++June 2022 LW: add in 'carmaliqopt' for CARMA clouds ++++++++++++++++++++++++++
            case ('carmaliqopt')
               call liquid_cloud_get_rad_props_lw_carma(state, pbuf, liq_lw_abs)
! ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            case default
...
! ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
         ! Output cloud optical depth fields for the longwave band (3.5 µm)
         rd%liq_icld_lwabs_3451(:ncol,:) = liq_lw_abs(16,:ncol,:)
! ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
! ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        ! add fillvalue for night columns (only for testing lw opt; for real, there's no night fillvalue for lw)
        ! comment out this section for real starting here
         do i = 1, Nnite
            rd%liq_icld_lwabs_3451(IdxNite(i),:)   = fillvalue
         end do
        ! comment out this section for real stopping here
! ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
! ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
   call outfld('LIQ_ICLD_VISTAU3451', rd%liq_icld_vistau_3451, pcols, lchnk)
   call outfld('LIQ_ICLD_VISSSA3451', rd%liq_icld_visssa_3451, pcols, lchnk)
   call outfld('LIQ_ICLD_LWABS3451', rd%liq_icld_lwabs_3451, pcols, lchnk)
! ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
   if (cldfsnow_idx > 0) then
