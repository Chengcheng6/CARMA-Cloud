...
subroutine hetnucl(carma, cstate, iz, rc)
  ...
  ! Local declarations
  logical                              :: evapfrom_nucto !(LW: added for homo. & hetero. nuc., Sep 2022)
  ...
  integer                              :: inucto  ! index of target nucleation bin (LW: added for homo. & hetero. nuc., Sep 2022)
  real(kind=f)                         :: rmw
  real(kind=f)                         :: R_H2O
  real(kind=f)                         :: rnh2o
  real(kind=f)                         :: rlogs
  real(kind=f)                         :: ag
  real(kind=f)                         :: contang ! contact angle
  real(kind=f)                         :: xh
  real(kind=f)                         :: phih
  real(kind=f)                         :: rath
  real(kind=f)                         :: fv3h
  ...
  real(kind=f), parameter              :: gdes    = 4.125e-13_f   ! From Chen et al., ACP, 2008 average of Saharan and Asian dust  
  !real(kind=f), parameter              :: gsd     = 2.9e-14_f
  real(kind=f), parameter              :: gsd     = 0.1_f * gdes
  real(kind=f), parameter              :: zeld    = 0.1_f
  real(kind=f), parameter              :: vibfreq = 1.e13_f
  real(kind=f), parameter              :: diflen  = 0.1e-7_f
  real(kind=f)                         :: rmiv    ! cosine of contact angle?
      
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ! LW (Sep 2022): modified for homo. & hetero. nuc.
 real(kind=f)                         :: rmiv_b 
 real(kind=f)                         :: rmiv_c

! CM:
! For contact angle, use the Traner et al. (2008) parameterization in the 150K to 240K temperature range,
! For temperatures less than 150K, set to the Trainer 150K value (this shouldn't be a problem in the TTL)
! For temperatures warmer than 240K, set the contact angle to the Chen et al. (2008) value      

  if (t(iz) .gt. 240._f) then
!    rmiv    = 0.95_f
!    rmiv    = 0.9945_f              ! From Chen et al., ACP, 2008 for average of Saharan and Asian dust
     rmiv    = 0.97_f
  else
    rmiv = 0.94_f - (6005._f * exp(-0.065_f * max(150._f, t(iz))))  ! Trainer et al. (2008) parameterization for the 150K to 240K range
...
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
              ...
              !
              ! CM NOTE: We are only trying to model ice partcles, so turn on nucleation
              ! where the CARMA microphysics takes over.
              ...
              if ((t(iz) .lt. T0) .and. (supsati(iz,igas) .gt. 0._f)) then
