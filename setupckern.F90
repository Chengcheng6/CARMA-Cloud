...
!  The data were compiled by Hall (J. Atmos. Sci. 37, 2486-2507, 1980).
          ...
          if( icoag(j1,j2) .ne. 0 )then
            ...
            do i1 = 1, NBIN
              ...
              do i2 = 1, NBIN
                ...
                x_coal = max(x_coal,0._f)   ! +++ Updated lower limit
                e_coal = min(x_coal,1._f)
 
                e_coll = min(e_coll,1._f)   ! +++ Added here to avoid some bad values  
  ...
  !                 endif
                ...
                !+++
                ! test cgr/4 for any r < 50 um
                if (r1 .lt. 50.e-4_f .or. r2 .lt. 50.e-4_f) then
                    cgr = cgr/4._f
                endif
                ! Now combine all the coagulation and collection kernels into the
                ! overall kernel.
                !ckernel(k,i1,i2,j1,j2) = cbr + ccd + cgr
                ckernel(k,i1,i2,j1,j2) = cbr + cgr          !+++ for ccd = 0
