      ...
      do jfrom = 1,nnucbin(igfrom,ibin,igroup)
        ...
        if( pconmax(iz,igfrom) .gt. FEW_PC )then
          ...
          if( rnuclg(ifrom,igfrom,igroup) .gt. 0._f )then
            ...
            rnucprod_nd = rnuclg(ifrom,igfrom,igroup) * pc(iz,ifrom,iefrom)

             !!!!!! Parse out the homogeneous nucleation from freezing on sulfuric acid (dust and pure water are handled in downgxfer.F90) <--- CMALONEY

!if ( rnucprod_nd .gt. 0._f) then
!write(LUNOPRT,*) ' '
!write(LUNOPRT,*) ' source bin (ifrom) =', ifrom
!write(LUNOPRT,*) ' target bin (ibin)=', ibin
!write(LUNOPRT,*) ' source element (iefrom) =', iefrom
!write(LUNOPRT,*) ' target element (ielem) =', ielem
!write(LUNOPRT,*) ' source group (igfrom) =', igfrom
!write(LUNOPRT,*) ' target group (igroup) = ', igroup
!write(LUNOPRT,*) ' rnucprod_nd =', rnucprod_nd
!end if

               pc_so4(iz,ibin,ielem) = pc_so4(iz,ibin,ielem) + rnucprod_nd
