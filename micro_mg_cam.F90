...
subroutine micro_mg_cam_register
   ...
   !print*, 'Mod micro_mg_cam.F90, Sub micro_mg_cam_register, do_cldliq', do_cldliq
   If ( .not. do_cldliq ) Then
      call pbuf_add_field('RE_LIQ',     'physpkg',dtype_r8,(/pcols,pver/), re_liq_idx)
   EndIf
...
subroutine micro_mg_cam_init(pbuf2d)
   ...
   case (2:3)
      ...
      call micro_mg_init3_0( &
           ...
           microp_uniform, &
           do_cldliq, &    ! Jamison A Smith (JAS) 2020 Aug 21.  ! Cheng 202402, changed to lowercase. 
           do_cldice, use_hetfrz_classnuc, &
           micro_mg_precip_frac_method, micro_mg_berg_eff_factor, &
...
!Hail/Graupel Process Rates                
   ...
   real(r8) :: packed_re_liq(mgncol,nlev)  ! JAS 2020 Feb 24
...
!Hail/Graupel Process Rates                
   ...
   real(r8), pointer, dimension(:,:) :: re_liq       ! JAS 2020 Feb 24, liq effective radius (m)
...
!  Averaging arrays for supercooled liquid
   ...
   ! + JAS 2020 Feb 24, The flags are MG-centric. Is MG doing the cldliq?
   ! If MG is not doing the cldliq, then Carma is doing the cldliq

   if ( .not. do_cldliq ) then
      call pbuf_get_field ( pbuf, re_liq_idx, re_liq, col_type=col_type, copy_if_needed=use_subcol_microp)
   end if
...
! Graupel/Hail process rates
   ...
   If ( .not. do_cldliq ) Then
      packed_re_liq = packer % pack ( re_liq )
   EndIf
   ...
   do it = 1, num_steps
      ...
      case (1)
         ...
         case (0)
            call micro_mg_tend1_0( &
                 ...
                 packed_nfice, packed_prer_evap &
                 , do_cldice &      ! JAS 2020 Feb 24
                 , do_cldliq &
                 , errstring,                      &
                 packed_tnd_qsnow, packed_tnd_nsnow  &
                 , packed_re_ice  &
                 , packed_re_liq  &  ! JAS 2020 Feb 24
      ...
      ! Commented by JAS. Noted by Cheng.
      !write ( *, * ) 'micro_mg_cam, packed_qvlat'
      !write ( *, '( 3x, 1p, 4e11.2 )' )  packed_qvlat ( 1, 1:4)
      !write ( *, '( 3x, 1p, 4e11.2 )' )  packed_qvlat ( 1, 29:32)
      !print*

      !LogicalResult = Allocated ( ptend_loc % q )
      !print*, 'micro_mg_cam, Allocated ( ptend_loc % q )', LogicalResult
      !print*
      ...
      ! +++ 
      !print*, 'micro_mg_cam bounds', lbound ( ptend_loc % q, 1 ), lbound ( ptend % q, 1 )
      
      !print*, 'micro_mg_cam, ptend_loc % q ( 1, 12, 1 )', ptend_loc % q ( 1, 12, 1 )
      !print*, 'micro_mg_cam, ptend % q ( 1, 12, 1 )', ptend % q ( 1, 12, 1 )
      !print*
      ...
      ! +++
      !print*, 'micro_mg_cam, ptend_loc % q ( :, :, 1 )', ptend_loc % q ( :, :, 1 )
      !print*

      !print*, 'micro_mg_cam, state_loc % q ( :, :, 1 )', state_loc % q ( :, :, 1 )
      !print*
   ...
   !print*, 'micro_mg_cam, End of it = ', it

   end do  ! it = 1, num_steps
