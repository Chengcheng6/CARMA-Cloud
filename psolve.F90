...
subroutine psolve(carma, cstate, iz, ibin, ielem, rc)
  ...
  if(do_grow) then
    ...
    ! Activate droplets according to an externally specified droplet activation
    ! rate. The drops that are being activated are external to CARMA and thus
    ! any mass needed comes from the vapor only, which will be determined in gsolve.
    !
    ! NOTE: CAM also changes droplet number based upon changes in cloud fraction and
    ! regeneration. Here we only want the positive side of this number.
    if (actpe(iz, ibin, ielem) .gt. 0._f) then
      ppd = ppd + actpe(iz, ibin, ielem)
    end if
