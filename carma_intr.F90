...
module carma_intr
  ...
  public carma_output_budget_diagnostics ! calculate and output model specific aerosol/cloud budget terms
  ...
  integer, parameter :: NGPDIAGS    = 29  ! Number of particle diagnostics ...
  ...
  integer, parameter :: GPDIAGS_NC  = 28  ! Number Concentraion [#/kg] (LW 07/2023)
  integer, parameter :: GPDIAGS_MR0  = 29  ! Mass Mixing Ratio before CARMA timestep (LW 07/2023)

  ! Particle Bin Statistics
...
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  integer, parameter             :: NNUCDIAGS         = 2
  integer, parameter             :: NUCDIAGS_SO4CCN   = 1         ! Number of sulfate particles which nucleated
  integer, parameter             :: NUCDIAGS_DSTCCN   = 2         ! Number of dust particles which nucleated
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  ! Gas Statistics
  integer, parameter :: NGSDIAGS   = 21  ! Number of gas diagnostics ...
  ...
  integer, parameter :: GSDIAGS_WT = 5  ! weight percent composition for aerosol
  integer, parameter :: GSDIAGS_C1 = 6  !
  integer, parameter :: GSDIAGS_C2 = 7  !
  integer, parameter :: GSDIAGS_C3 = 8  !
  integer, parameter :: GSDIAGS_T1 = 9  !
  integer, parameter :: GSDIAGS_T2 = 10  !
  integer, parameter :: GSDIAGS_T3 = 11  !
  integer, parameter :: GSDIAGS_AW1 = 12  !
  integer, parameter :: GSDIAGS_AW2 = 13  !
  integer, parameter :: GSDIAGS_AW3 = 14  !
  integer, parameter :: GSDIAGS_F1 = 15  !
  integer, parameter :: GSDIAGS_F2 = 16  !
  integer, parameter :: GSDIAGS_F3 = 17  !
  integer, parameter :: GSDIAGS_LC = 18
  integer, parameter :: GSDIAGS_R1 = 19
  integer, parameter :: GSDIAGS_R2 = 20
  integer, parameter :: GSDIAGS_R3 = 21
  ...
  integer                        :: ipbuf4prain                   ! physics buffer index for a carma prain (LW 08/2023)
  integer                        :: ipbuf4evapr                   ! physics buffer index for a carma evapr (LW 08/2023)
  integer                        :: ipbuf4cldgrp                  ! physics buffer index for a carma in-cloud graupel mixing ratio Zhu Sep 12 2024
  integer                        :: ipbuf4degrp                   ! physics buffer index for a carma graupel effective diameter  Zhu Sep 12 2024
...
contains
  ...
  !! the gases are linked; otherwise, the gas is added to CAM. The shortname
...
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ! LW (08/2023): Add for determining precip and evap tendency
    call pbuf_add_field('CRPRAIN_micmac', 'global',dtype_r8, (/pcols, pver/), ipbuf4prain)
    call pbuf_add_field('CRNEVAPR_micmac', 'global',dtype_r8, (/pcols, pver/), ipbuf4evapr)
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

    !!---------------------------- Zhu Sep 12 2024 ----------------------------
    ! add pbuf for graupel in-cloud mass mixing ratio (kg/kg) and effective diameter (m)
    call pbuf_add_field('ICGRPCARMA', 'global',dtype_r8, (/pcols, pver/), ipbuf4cldgrp)
    call pbuf_add_field('DEGCARMA', 'global',dtype_r8, (/pcols, pver/), ipbuf4degrp)
    !!----------------------------------------------------------------------------
    ...
    !
    ! NOTE: We only want to do this with RRTMG. If CAM_RT is being used, then
    ! skip this.
    ...
    if ((masterproc) .and. (initial_run) .and. (radiation_scheme == "rrtmg") .and. (carma_do_optics)) then
      !+++++++++++++++++ Add in Carma_CreateCloudOpticsFile (June 2022, LW) ++++++++++++++++++
      !call Carma_CreateOpticsFile (carma, rc)
      !write(*,*) 'Carma_CreateCloudOpticsFile is used'
      call CARMA_CreateCloudOpticsFile (carma, rc)

      !if (rc < 0) call endrun ('carma_intr.F90 carma_register ::' &
      !// ' Carma_CreateOpticsFiles failed.')
      if (rc < 0) call endrun ('carma_intr.F90 carma_register ::' &
      // ' Carma_CreateCloudOpticsFiles failed.')
    end if
      !+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    return
...
2   format(i6,4x,3(1pe11.3,4x))
    ...
    ! Add these for debug purpises
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ! LW (07/2023): Add the following to check if (so4_a1+so4_a3) and 
    ! (dst_a1+dst_a3) from CAM matches CRCNMR and CRDUSTMR before CARMA timestep
    call addfld('MAMsulfate_acco', (/ 'lev' /), 'A', 'kg/kg', 'Sulfate mass mixing ratio (accum+coarse)')
    call addfld('MAMdust_acco', (/ 'lev' /), 'A', 'kg/kg', 'Dust mass mixing ratio (accum+coarse)')
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
! LW (08/2023): Add CARMA fields for precip and evaporation tendency computed by CARMA
	call addfld ('PRODPREC_CARMA',   (/ 'lev' /), 'A', 'kg/kg/s',  'Rate of conversion of condensate to precip')
    call addfld ('EVAPPREC_CARMA',   (/ 'lev' /), 'A', 'kg/kg/s',  'Rate of evaporation of falling precip')      
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    do igroup = 1, NGROUP
      ...
      call addfld(trim(sname)//'NC', (/ 'lev' /), 'A', '#/kg', trim(sname) // ' number concentration')    !(LW 07/2023)
      call addfld(trim(sname)//'MR0', (/ 'lev' /), 'A', 'kg/kg', trim(sname) // ' mass mixing ratio before CARMA timestep')  !(LW 07/2023)

      ! The following are group diagnostics for AD, VD, RE, MR, ND in small, medium, and large size ranges
      call addfld(trim(sname)//'ADS', (/ 'lev' /), 'A', 'um2/cm3', trim(sname) // ' surface area density, small')
      call addfld(trim(sname)//'VDS', (/ 'lev' /), 'A', 'um3/cm3', trim(sname) // ' volume density, small')
      call addfld(trim(sname)//'RES', (/ 'lev' /), 'A', 'um', trim(sname) // ' effective radius, small')
      call addfld(trim(sname)//'MRS', (/ 'lev' /), 'A', 'kg/kg', trim(sname) // ' mass mixing ratio, small')
      call addfld(trim(sname)//'NDS', (/ 'lev' /), 'A', '#/cm3', trim(sname) // ' number density, small')
      call addfld(trim(sname)//'ADM', (/ 'lev' /), 'A', 'um2/cm3', trim(sname) // ' surface area density, medium')
      call addfld(trim(sname)//'VDM', (/ 'lev' /), 'A', 'um3/cm3', trim(sname) // ' volume density, medium')
      call addfld(trim(sname)//'REM', (/ 'lev' /), 'A', 'um', trim(sname) // ' effective radius, medium')
      call addfld(trim(sname)//'MRM', (/ 'lev' /), 'A', 'kg/kg', trim(sname) // ' mass mixing ratio, medium')
      call addfld(trim(sname)//'NDM', (/ 'lev' /), 'A', '#/cm3', trim(sname) // ' number density, medium')
      call addfld(trim(sname)//'ADL', (/ 'lev' /), 'A', 'um2/cm3', trim(sname) // ' surface area density, large')
      call addfld(trim(sname)//'VDL', (/ 'lev' /), 'A', 'um3/cm3', trim(sname) // ' volume density, large')
      call addfld(trim(sname)//'REL', (/ 'lev' /), 'A', 'um', trim(sname) // ' effective radius, large')
      call addfld(trim(sname)//'MRL', (/ 'lev' /), 'A', 'kg/kg', trim(sname) // ' mass mixing ratio, large')
      call addfld(trim(sname)//'NDL', (/ 'lev' /), 'A', '#/cm3', trim(sname) // ' number density, large')

!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ! LW (Sep 2022): added for hetero. nuc.
    !!!! CMALONEY: Not sure if this is where this should go, but add Homo and Het nuc rates as well is IN
!      call addfld(trim(sname)//'HOMA', '#/cm3', pver, 'A', 'Number density of nucleated SO4 - avg', phys_decomp)
!      call addfld(trim(sname)//'HETA', '#/cm3', pver, 'A', 'Number density of nucleated Dust - avg', phys_decomp)
!      call addfld(trim(sname)//'HOMI', '#/cm3', pver, 'I', 'Number density of nucleated SO4 - instant', phys_decomp)
!      call addfld(trim(sname)//'HETI', '#/cm3', pver, 'I', 'Number density of nucleated Dust - instant', phys_decomp)
      call addfld(trim(sname)//'HOMA', (/ 'lev' /), 'A', '#/cm3', 'Number density of nucleated SO4 - avg')
      call addfld(trim(sname)//'HETA', (/ 'lev' /), 'A', '#/cm3', 'Number density of nucleated Dust - avg')
      call addfld(trim(sname)//'HOMI', (/ 'lev' /), 'I', '#/cm3', 'Number density of nucleated SO4 - instant')
      call addfld(trim(sname)//'HETI', (/ 'lev' /), 'I', '#/cm3', 'Number density of nucleated Dust - instant')
    !!!!!!!!
      ...
      if (history_carma) then
         ...
         call add_default(trim(sname)//'NC', 1, ' ')    !(LW 07/2023)
         call add_default(trim(sname)//'MR0', 1, ' ')    !(LW 07/2023)
    ...
    do igas = 1, NGAS
      ...
      call addfld(trim(cnst_name(icnst))//'C1', (/ 'lev' /), 'A', '#/cm3',       &
           trim(cnst_name(icnst)) // ' cloudborne black carbon num')
      call addfld(trim(cnst_name(icnst))//'C2', (/ 'lev' /), 'A', '#/cm3',       &
           trim(cnst_name(icnst)) // ' cloudborne dust a1 num')
      call addfld(trim(cnst_name(icnst))//'C3', (/ 'lev' /), 'A', '#/cm3',       &
           trim(cnst_name(icnst)) // ' cloudborne dust a2 num')
            call addfld(trim(cnst_name(icnst))//'T1', (/ 'lev' /), 'A', '#/cm3',       &
           trim(cnst_name(icnst)) // ' intestitial black carbon num')
      call addfld(trim(cnst_name(icnst))//'T2', (/ 'lev' /), 'A', '#/cm3',       &
           trim(cnst_name(icnst)) // ' intestitial dust a1 num')
      call addfld(trim(cnst_name(icnst))//'T3', (/ 'lev' /), 'A', '#/cm3',       &
           trim(cnst_name(icnst)) // ' intestitial dust a2 num')
      call addfld(trim(cnst_name(icnst))//'AW1', (/ 'lev' /), 'A', 'mug m-3',       &
           trim(cnst_name(icnst)) // ' modal added mass1')
      call addfld(trim(cnst_name(icnst))//'AW2', (/ 'lev' /), 'A', 'mug m-3',       &
           trim(cnst_name(icnst)) // ' modal added mass2')
      call addfld(trim(cnst_name(icnst))//'AW3', (/ 'lev' /), 'A', 'mug m-3',       &
           trim(cnst_name(icnst)) // ' modal added mass3')
      call addfld(trim(cnst_name(icnst))//'F1', (/ 'lev' /), 'A', '1',       &
           trim(cnst_name(icnst)) // ' (OC+BC)/(OC+BC+SO4)')
      call addfld(trim(cnst_name(icnst))//'F2', (/ 'lev' /), 'A', '1',       &
           trim(cnst_name(icnst)) // ' (OC+BC)/(OC+BC+SO4)')
      call addfld(trim(cnst_name(icnst))//'F3', (/ 'lev' /), 'A', '1',       &
           trim(cnst_name(icnst)) // ' (OC+BC)/(OC+BC+SO4)')
      call addfld(trim(cnst_name(icnst))//'LC', (/ 'lev' /), 'A', '1',       &
           trim(cnst_name(icnst)) // ' stratform fraction')
      if (history_carma) then
    ...
    ! Add subgrid saturation factor.
    if (carma_do_grow) then
      call addfld('CRQSATFAC',  (/ 'lev' /), 'A', ' ', 'subgrid saturation factor - input to CARMA')      
    end if
  ...
  !! @author  Chuck Bardeen
  !! @version May-2009
  subroutine carma_timestep_tend(state, cam_in, cam_out, ptend, dt, pbuf, dlf, &
                  det_cnd, det_ice, prec_str, snow_str, prec_sed, snow_sed, ustar, obklen)
...
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    real(r8), intent(out), optional      :: det_cnd(pcols)  !! total detrained condensate (m/s)
    real(r8), intent(out), optional      :: det_ice(pcols)  !! total detrained ice (m/s)
    ...
    type (carma_type), pointer :: carma_ptr  ! the carma object
    ...
    real(r8) :: bndiags(pcols, pver, NBIN, NELEM, NBNDIAGS)
    ...
    real(r8) :: arat(NBIN)     ! particle area ratio ()
    ...
    real(r8) :: ads(pver)       ! area density, small (um2/cm3)
    real(r8) :: vds(pver)       ! volume density, small (um3/cm3)
    real(r8) :: mrs(pver)       ! mmr, small (kg/kg)
    real(r8) :: res(pver)       ! effective radius, small (um)
    real(r8) :: nds(pver)       ! number density, small (cm-3)
    real(r8) :: adm(pver)       ! area density, medium (um2/cm3)
    real(r8) :: vdm(pver)       ! volume density, medium (um3/cm3)
    real(r8) :: mrm(pver)       ! mmr, medium (kg/kg)
    real(r8) :: rem(pver)       ! effective radius, medium (um)
    real(r8) :: ndm(pver)       ! number density, medium (cm-3)
    real(r8) :: adl(pver)       ! area density, large (um2/cm3)
    real(r8) :: vdl(pver)       ! volume density, large (um3/cm3)
    real(r8) :: mrl(pver)       ! mmr, large (kg/kg)
    real(r8) :: rel(pver)       ! effective radius, large (um)
    real(r8) :: ndl(pver)       ! number density, large (cm-3)
    real(r8) :: nc(pver)        ! number concentration (kg-1)    !(LW 07/2023)
    real(r8) :: mr0(pver)       ! mass mixing ratio before CARMA timestep (kg/kg) !(LW 07/2023)
    ...
    real(r8) :: re_small_limit                ! upper limit for the small size range
    real(r8) :: re_large_limit                ! lower limit for the large size range 
    real(kind=f) :: rhoa_wet(pver)            ! air density (g/cm3)  (LW 07/2023: added for number conc.) 
    ...
    real(r8), pointer, dimension(:,:) :: ast       ! stratisform cloud fraction (fraction)
    real(r8), pointer, dimension(:,:) :: qsatfac   ! subgrid saturation factor ()
    real(r8), pointer, dimension(:,:) :: re_ice    ! ice effective radius (m)
    real(r8), pointer, dimension(:,:) :: re_liq    ! liq effective radius (m)
    real(r8), pointer, dimension(:,:) :: clubb_kvh ! m2/s

    ! zhu 20220919
    real(r8), pointer, dimension(:,:) :: immbccb !cloud borne passing from MAM
    real(r8), pointer, dimension(:,:) :: immd1cb
    real(r8), pointer, dimension(:,:) :: immd2cb
    real(r8), pointer, dimension(:,:) :: immbcit !interstitial passing from MAM
    real(r8), pointer, dimension(:,:) :: immd1it
    real(r8), pointer, dimension(:,:) :: immd2it   
    real(r8), pointer, dimension(:,:) :: awcambc !modal added mass [mug m-3]
    real(r8), pointer, dimension(:,:) :: awcamd1
    real(r8), pointer, dimension(:,:) :: awcamd2
    real(r8), pointer, dimension(:,:) :: awfacmbc !(OC+BC)/(OC+BC+SO4)
    real(r8), pointer, dimension(:,:) :: awfacmd1
    real(r8), pointer, dimension(:,:) :: awfacmd2
    real(r8), pointer, dimension(:,:) :: lcldm    !Stratiform cloud fraction
    real(r8), pointer, dimension(:,:) :: hetrbc   !mass mean radius
    real(r8), pointer, dimension(:,:) :: hetrd1
    real(r8), pointer, dimension(:,:) :: hetrd2

    ! zhu 20220930
    real(r8) :: immbccb_out(pver)
    real(r8) :: immd1cb_out(pver)
    real(r8) :: immd2cb_out(pver)
    real(r8) :: immbcit_out(pver)
    real(r8) :: immd1it_out(pver)
    real(r8) :: immd2it_out(pver)
    real(r8) :: awcambc_out(pver)
    real(r8) :: awcamd1_out(pver)
    real(r8) :: awcamd2_out(pver)
    real(r8) :: awfacmbc_out(pver)
    real(r8) :: awfacmd1_out(pver)
    real(r8) :: awfacmd2_out(pver)
    real(r8) :: ast_out(pver)
    real(r8) :: hetrbc_out(pver)
    real(r8) :: hetrd1_out(pver)
    real(r8) :: hetrd2_out(pver)
    real(r8) :: lcldm_out(pver)
...
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    ! Indexing variables for modes and species (species such as so4, dust, sea salt)
    integer :: m, nn, ii, l
    ...
    real(r8) :: mamso4_mmr_acco(pcols, pver)
    real(r8) :: mamdust_mmr_acco(pcols, pver)    
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ! LW (Sep 2022): added for hetero. nuc.
    !!! Nucleation statistics variables - CMALONEY !!!!
    real(r8)              :: so4_ac_frac(pcols,pver,NBIN)
    real(r8)              :: so4_co_frac(pcols,pver,NBIN)
    real(r8)              :: dst_ac_frac(pcols,pver,NBIN)
    real(r8)              :: dst_co_frac(pcols,pver,NBIN)

    real(r8)              :: nucdiags(pcols, pver, NELEM, NGROUP, NNUCDIAGS)
!    real(r8)              :: homjn(pver)                  ! homogeneous nucleation rate #/cm3/sec
!    real(r8)              :: hetjn(pver)                  ! heterogenous nucleation rate #/cm3/sec    
    real(r8)              :: so4Nucleated(pver)
    real(r8)              :: dstNucleated(pver)
    real(r8)              :: so4core(pver)
    real(r8)              :: dstcore(pver)
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ! LW (08/2023): Add for determining precip and evap tendency
    ! Note: The follows variables and pointers are CAM variables and pointers.
    ! Therefore, they have different dimensions from the corresponding CARMA global variables.
    real(r8)              :: prain(pcols,pver)  ! Precip Rate(Rain+Snow) corresponding to the final 'prain' in micro_mg_2_0.F90, g/cm^3/s
    real(r8)              :: nevapr(pcols,pver) ! Evaporation Rate(Rain+Snow) corresponding to the final 'nevapr' in micro_mg_2_0.F90, g/cm^3/s
    ! physics buffer 
    real(r8), pointer, dimension(:,:)   :: prain_ptr
    real(r8), pointer, dimension(:,:)   :: evapr_ptr
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    ! Cloud pbuf fields
    call pbuf_get_field(pbuf, pbuf_get_index("AST"), ast)
    call pbuf_get_field(pbuf, pbuf_get_index("QSATFAC"), qsatfac)
    ...
    if (carma_do_cldliq) then
      call pbuf_get_field(pbuf, pbuf_get_index("RE_LIQ"), re_liq)
      call pbuf_get_field(pbuf,pbuf_get_index("kvh"),clubb_kvh)
    endIf

    ! Zhu 20220919 read in the immerse freezing nucleation probablity
    if ( carma_do_cldliq ) then
      call pbuf_get_field ( pbuf, pbuf_get_index("IMBCCB"), immbccb)
      call pbuf_get_field ( pbuf, pbuf_get_index("IMD1CB"), immd1cb)
      call pbuf_get_field ( pbuf, pbuf_get_index("IMD2CB"), immd2cb)
      call pbuf_get_field ( pbuf, pbuf_get_index("IMBCIT"), immbcit)
      call pbuf_get_field ( pbuf, pbuf_get_index("IMD1IT"), immd1it)
      call pbuf_get_field ( pbuf, pbuf_get_index("IMD2IT"), immd2it)
      call pbuf_get_field ( pbuf, pbuf_get_index("AWCAMBC"), awcambc)
      call pbuf_get_field ( pbuf, pbuf_get_index("AWCAMD1"), awcamd1)
      call pbuf_get_field ( pbuf, pbuf_get_index("AWCAMD2"), awcamd2)
      call pbuf_get_field ( pbuf, pbuf_get_index("AWFACMBC"), awfacmbc)
      call pbuf_get_field ( pbuf, pbuf_get_index("AWFACMD1"), awfacmd1)
      call pbuf_get_field ( pbuf, pbuf_get_index("AWFACMD2"), awfacmd2)
      call pbuf_get_field ( pbuf, pbuf_get_index("LCLDM"), lcldm)
      call pbuf_get_field ( pbuf, pbuf_get_index("HETRBC"),hetrbc)
      call pbuf_get_field ( pbuf, pbuf_get_index("HETRD1"),hetrd1)
      call pbuf_get_field ( pbuf, pbuf_get_index("HETRD2"),hetrd2)
    endif
...
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     ! Species indice (only get coarse mode until everything is working correctly)
      ...
      ! Get aerosol species mmr from MAM
      ! Sulfates
      ...
      ! Get sum sulfate and dust mmr from accum and coarse modes
      mamso4_mmr_acco = so4_mmr_ac + so4_mmr_co
      mamdust_mmr_acco = dust_mmr_ac + dust_mmr_co
      
      call outfld('MAMsulfate_acco', mamso4_mmr_acco(:,:), pcols, lchnk)
      call outfld('MAMdust_acco', mamdust_mmr_acco(:,:), pcols, lchnk)            
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    if (do_fixedinit) then
      ...
      if (single_column) then
        ...
        !
        ! NOTE: plat-1 causes Floating Point Exception when compiling the Scam
        ! for Docker.
        dx = 360._r8 / plon
        dy = 180._r8 / max (1, plat-1)
      end if
    ...
    do icol = 1, state_loc%ncol
      ...
      if (single_column) then
        ...
        if (abs(state_loc%lat(icol) /  DEG2RAD) >= (90._r8 - (90._r8 / max(1, plat-1)))) then
          ...
          do ibin = 1, NBIN
            ...
            if (ibin <= maxbin) then
              call CARMASTATE_SetBin(cstate, ielem, ibin, &
                   state_loc%q(icol, :, icnst4elem(ielem, ibin)), rc, clubb_kvh = clubb_kvh(icol,:))
              if (rc < 0) call endrun('carma_timestep_tend::CARMASTATE_SetBin failed.')
            else
              newstate(:) = 0._f
              
              call CARMASTATE_SetBin(cstate, ielem, ibin, newstate, rc, clubb_kvh = clubb_kvh(icol,:))
              if (rc < 0) call endrun('carma_timestep_tend::CARMASTATE_SetBin failed.')
            end if
          end do
        end if
      ...
      do igas = 1, NGAS
        ...
        call CARMASTATE_SetGas(cstate, igas, state_loc%q(icol, :, icnst4gas(igas)), rc &
          ...
          immbccb=immbccb(icol,:),immd1cb=immd1cb(icol,:),immd2cb=immd2cb(icol,:),&
          immbcit=immbcit(icol,:),immd1it=immd1it(icol,:),immd2it=immd2it(icol,:),&
          awcambc=awcambc(icol,:),awcamd1=awcamd1(icol,:),awcamd2=awcamd2(icol,:),&
          awfacmbc=awfacmbc(icol,:),awfacmd1=awfacmd1(icol,:),&
          awfacmd2=awfacmd2(icol,:),lcldm=lcldm(icol,:),&
          hetrbc=hetrbc(icol,:),hetrd1=hetrd1(icol,:),hetrd2=hetrd2(icol,:))
        if (rc < 0) call endrun('carma_timestep_tend::CARMASTATE_SetGas failed.')
      end do  ! igas

      !  JAS 2020 Jun 21: I have turned off the mass fixer in
      ! Carma_DiagnoseBins, so prec_str is not modified
      
      call CARMA_DiagnoseBins(carma, cstate, state_loc, pbuf, icol, dt, rc, &
                             dlf=dlf, prec_str=prec_str, snow_str=snow_str, &
                             so4_ac_frac=so4_ac_frac, so4_co_frac=so4_co_frac, &
                             dst_ac_frac=dst_ac_frac, dst_co_frac=dst_co_frac)
!                            !(LW: modified for homo. & hetero. nuc., Sep 2022)
      if (rc < 0) call endrun('carma_intr::carma_timestep_tend - Carma_DiagnoseBins failed.')
...
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      ...
      do ielem = 1, NELEM
        ...
        call CARMAELEMENT_Get(carma, ielem, rc, igroup=igroup)
        if (rc < 0) call endrun('carma_intr::carma_timestep_tend - CarmaElement_Get failed.')
        ...
        ! Gridbox average
        gpdiags(icol, :, igroup, GPDIAGS_MR0) = mr0        
      end do
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

      ! If the model supports detraining of condensed water from convection, then pass along the condensed H2O.
      call CARMA_Get(carma, rc, do_detrain=do_detrain)
      if (rc < 0) call endrun('carma_intr::carma_timestep_tend - CARMA_Get failed.')

      if (do_detrain) then
        call CARMA_Detrain(carma, cstate, cam_in, dlf, det_cnd, det_ice, state_loc, icol, dt, rc)
        if (rc < 0) call endrun('carma_intr::carma_timestep_tend - CARMA_Detrain failed.')
      end if
      

      ! For dry deposition, provide a surface friction velocity and an
      ! aerodynamic resistance for each of the land surface types. The values
      ! for the land come from the land model, but those for ocean and sea ice
      ...
      call CARMASTATE_Step (cstate, rc,cldfrc=ast(icol, :), qsatfac=qsatfac(icol, :), &
      ...
      if (rc < 0) call endrun ('carma_intr::carma_timestep_tend - CarmaState_Step failed.')
      
     ! Get the results for the CARMA particles.
      !
      ! For diagnostic groups, a special routine needs to be called to
      ! determine how bins affect the bulk state, since there is not an
      ! individual constituent for each bin.
      call CARMA_DiagnoseBulk(carma, cstate, cam_out, state_loc, pbuf, ptend, &
                              icol, dt, rc, dlf=dlf, prec_str=prec_str, &
                              snow_str=snow_str, prec_sed=prec_sed, snow_sed=snow_sed, &
                              re_ice=re_ice, &
                              so4_ac_frac=so4_ac_frac, so4_co_frac=so4_co_frac, &
                              dst_ac_frac=dst_ac_frac, dst_co_frac=dst_co_frac)
      if (rc < 0) call endrun ('carma_intr::carma_timestep_tend - Carma_DiagnoseBulk failed.')
      ...
      do ielem = 1, NELEM
        ...
        rel(:) = 0.0_r8
        ndl(:) = 0.0_r8
        nc(:)  = 0.0_r8    !(LW 07/2023)
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ! LW (Sep 2022): added for hetero. nuc.
        !!! CMALONEY: Sum nucleation rate across all bins
        so4core(:) = 0.0_r8
        dstcore(:) = 0.0_r8
        !!!!!!!    
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        ...
        do ibin = 1, NBIN
          call CARMASTATE_GetBin(cstate, ielem, ibin, newstate(:), rc, &
                 numberDensity=numberDensity, nucleationRate=nucleationRate, &
                 surface=dd, vd=vd, vf=vf, dtpart=dtpart, &
                 so4Nucleated=so4Nucleated, dstNucleated=dstNucleated, r_wet=r_wet)
!                !(LW: modified for homo. & hetero. nuc., Sep 2022)
          if (rc < 0) call endrun ('carma_intr::carma_timestep_tend - CarmaState_GetBin failed.')
          ...
          if (cnstType == I_CNSTTYPE_PROGNOSTIC) then
            ...
            if (ibin <= maxbin) then
              ...
              ptend%q(icol, :, icnst) = CARMA_SetWetToDry(state_loc, icol, (newstate(:) - state_loc%q(icol, :, icnst)) / dt, icnst)
...
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
         ! LW (07/2023): added for number conc.
         ! Get the air density.
...
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
          ...
          if (numberDensity(1) /= CAM_FILL) then
            ...
            !(LW 07/2023): numberDensity [/cm3], rhoa_wet [kg/m3], nc [/kg] 
            nc(:)  = nc(:)  + numberDensity(:) * 1.0e6_r8 / rhoa_wet(:)

            ! Use the wet rather than dry radius to calculate the area and volume
            ! densities in different size ranges.

            ! Use different size thresholds to partition cloud ice and liquid
            if (is_cloud) then
              if (is_ice) then
                re_small_limit = carma_re_ice_size_small
                re_large_limit = carma_re_ice_size_large
              else
                re_small_limit = carma_re_liquid_size_small
                re_large_limit = carma_re_liquid_size_large
              end if

              !+++ Cheng 20250801: modified codes for CARMA size cutoff.
              ! --- cumulative partition by radius ---------------------------------
              !   S bin : r_wet ≤ re_small_limit        (suffix s)
              !   M bin : r_wet ≤ re_large_limit        (suffix m)  (includes S bin)
              !   L bin : r_wet > re_large_limit        (suffix l)
              ! --------------------------------------------------------------------
              where (r_wet(:) .le. (re_small_limit / 1.e4_r8))
                ads(:) = ads(:) + numberDensity(:) * 4.0_r8 * PI * (r_wet(:)**2) * 1.0e8_r8
                vds(:) = vds(:) + numberDensity(:) * 4.0_r8 / 3.0_r8 * PI * (r_wet(:)**3)* 1.0e12_r8
                mrs(:) = mrs(:) + newstate(:)
                nds(:) = nds(:) + numberDensity(:)
              end where
              ! --------------------------------------------------------------------
              where (r_wet(:) .le. (re_large_limit / 1.e4_r8))
                adm(:) = adm(:) + numberDensity(:) * 4.0_r8 * PI * (r_wet(:)**2) * 1.0e8_r8
                vdm(:) = vdm(:) + numberDensity(:) * 4.0_r8 / 3.0_r8 * PI * (r_wet(:)**3)* 1.0e12_r8
                mrm(:) = mrm(:) + newstate(:)
                ndm(:) = ndm(:) + numberDensity(:)
              end where
              ! --------------------------------------------------------------------
              where (r_wet(:) .gt. (re_large_limit / 1.e4_r8))
                adl(:) = adl(:) + numberDensity(:) * 4.0_r8 * PI * (r_wet(:)**2) * 1.0e8_r8
                vdl(:) = vdl(:) + numberDensity(:) * 4.0_r8 / 3.0_r8 * PI * (r_wet(:)**3)* 1.0e12_r8
                mrl(:) = mrl(:) + newstate(:)
                ndl(:) = ndl(:) + numberDensity(:)
              end where
              ! --------------------------------------------------------------------
...
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ! LW (Sep 2022): added for homo. & hetero. nuc.
            !!! CMALONEY:
            !!! Get the number of aerosols  which participated in  nucleation
              so4core(:) = so4core(:) + so4Nucleated(:)

              dstcore(:) = dstcore(:) + dstNucleated(:)
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        ...
        if (numberDensity(1) /= CAM_FILL) then
          ...
          ! Calculate the effective radii in the size ranges in places where the
          ! mmr is greater than the threshold
          where (mrs(:) .le. carma_re_size_min_mmr)
            ads(:) = 0._r8
            vds(:) = 0._r8
            mrs(:) = 0._r8
            nds(:) = 0._r8
          end where
          where (mrm(:) .le. carma_re_size_min_mmr)
            adm(:) = 0._r8
            vdm(:) = 0._r8
            mrm(:) = 0._r8
            ndm(:) = 0._r8
          end where
          where (mrl(:) .le. carma_re_size_min_mmr)
            adl(:) = 0._r8
            vdl(:) = 0._r8
            mrl(:) = 0._r8
            ndl(:) = 0._r8
          ...
          gpdiags(icol, :, igroup, GPDIAGS_NC) = nc    !(LW 07/2023) 

          gpdiags(icol, :, igroup, GPDIAGS_ADS) = ads
          gpdiags(icol, :, igroup, GPDIAGS_VDS) = vds
          gpdiags(icol, :, igroup, GPDIAGS_RES) = res
          gpdiags(icol, :, igroup, GPDIAGS_MRS) = mrs
          gpdiags(icol, :, igroup, GPDIAGS_NDS) = nds
          gpdiags(icol, :, igroup, GPDIAGS_ADM) = adm
          gpdiags(icol, :, igroup, GPDIAGS_VDM) = vdm
          gpdiags(icol, :, igroup, GPDIAGS_REM) = rem
          gpdiags(icol, :, igroup, GPDIAGS_MRM) = mrm
          gpdiags(icol, :, igroup, GPDIAGS_NDM) = ndm
          gpdiags(icol, :, igroup, GPDIAGS_ADL) = adl
          gpdiags(icol, :, igroup, GPDIAGS_VDL) = vdl
          gpdiags(icol, :, igroup, GPDIAGS_REL) = rel
          gpdiags(icol, :, igroup, GPDIAGS_MRL) = mrl
          gpdiags(icol, :, igroup, GPDIAGS_NDL) = ndl
...
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ! LW (Sep 2022): added for homo. & hetero. nuc.
            !!! CMALONEY:
            !!! Get nucleaiton statistics, the element dimension here represents
            !!! the DESTINAATION element. Source elemetns are
            !!! weeded out in carmastate_mod.F90
             nucdiags(icol,:,ielem,igroup,NUCDIAGS_SO4CCN) = so4core
             nucdiags(icol,:,ielem,igroup,NUCDIAGS_DSTCCN) = dstcore
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++        
      end do
      ...
      do igas = 1, NGAS
        ...
        call CARMASTATE_GetGas(cstate, igas, newstate(:), rc, satice = satice, satliq = satliq &
              ...
              , immbccb_out=immbccb_out,immd1cb_out=immd1cb_out,immd2cb_out=immd2cb_out &
              , immbcit_out=immbcit_out,immd1it_out=immd1it_out,immd2it_out=immd2it_out &
              , awcambc_out=awcambc_out,awcamd1_out=awcamd1_out,awcamd2_out=awcamd2_out &
              , awfacmbc_out=awfacmbc_out,awfacmd1_out=awfacmd1_out,awfacmd2_out=awfacmd2_out &
              , lcldm_out=lcldm_out &
              , hetrbc_out=hetrbc_out,hetrd1_out=hetrd1_out,hetrd2_out=hetrd2_out)
        if (rc < 0) call endrun ('carma_intr::carma_timestep_tend - CarmaState_GetGas failed.')
        ...
        gsdiags(icol, :, igas, GSDIAGS_C1) = immbccb_out(:)      
        gsdiags(icol, :, igas, GSDIAGS_C2) = immd1cb_out(:)
        gsdiags(icol, :, igas, GSDIAGS_C3) = immd2cb_out(:)
        gsdiags(icol, :, igas, GSDIAGS_T1) = immbcit_out(:)
        gsdiags(icol, :, igas, GSDIAGS_T2) = immd1it_out(:)
        gsdiags(icol, :, igas, GSDIAGS_T3) = immd2it_out(:)
        gsdiags(icol, :, igas, GSDIAGS_AW1) = awcambc_out(:)
        gsdiags(icol, :, igas, GSDIAGS_AW2) = awcamd1_out(:)
        gsdiags(icol, :, igas, GSDIAGS_AW3) = awcamd2_out(:)
        gsdiags(icol, :, igas, GSDIAGS_F1) = awfacmbc_out(:)
        gsdiags(icol, :, igas, GSDIAGS_F2) = awfacmd1_out(:)
        gsdiags(icol, :, igas, GSDIAGS_F3) = awfacmd2_out(:)
        gsdiags(icol, :, igas, GSDIAGS_LC) = lcldm_out(:)
        gsdiags(icol, :, igas, GSDIAGS_R1) = hetrbc_out(:)
        gsdiags(icol, :, igas, GSDIAGS_R2) = hetrd1_out(:)
        gsdiags(icol, :, igas, GSDIAGS_R3) = hetrd2_out(:)
      ...
      if (carma_do_thermo) then 
        ptend%s(icol, :) = CARMA_SetWetToDry(state_loc, icol, (newstate(:) - state_loc%t(icol, :)) * cpair / dt)
      endif
...
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      ! LW (08/2023): Add for determining precip and evap tendency
      ! Get precip and evap tendency, and put them into pbuf

      call CARMASTATE_Get(cstate, rc, prain=prain(icol, :), nevapr=nevapr(icol, :))
      if (rc < 0) call endrun('carma_timestep_tend::CARMASTATE_Get failed.')

      ! put CARMA prain, nevapr to a 10-min pbuf
      call pbuf_get_field(pbuf, pbuf_get_index('CRPRAIN_micmac'), prain_ptr)
      call pbuf_get_field(pbuf, pbuf_get_index('CRNEVAPR_micmac'), evapr_ptr)
      
      prain_ptr(icol, :) = prain(icol, :)
      evapr_ptr(icol, :) = nevapr(icol, :)
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++  
...
!$OMP END CRITICAL
    ...
    call carma_output_diagnostics(state_loc, ptend, pbuf, cam_in, gpdiags, sbdiags, gsdiags, spdiags, bndiags, nucdiags)
!                            !(LW: modified for homo. & hetero. nuc., Sep 2022)

!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ! LW (08/2023): Add for determining precip and evap tendency
    ! Output precip and evap tendency.
    ! 'PRODPREC' and 'EVAPPREC' are from pbuf.
    ! 'PRODPREC_CARMA' and 'EVAPPREC_CARMA' are based on CARMA global variable.
    ! These two sets should match when we use CARMA microphysics.
    !call outfld('PRODPREC',    prain,       pcols, lchnk)
    !call outfld('EVAPPREC',    nevapr,      pcols, lchnk)      
    call outfld('PRODPREC_CARMA',    prain,       pcols, lchnk)
    call outfld('EVAPPREC_CARMA',    nevapr,      pcols, lchnk)      
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++  
...
!                  call endrun('carma_checkstate: ERROR -- Not enough mass available to fix shortfall.  -- '//trim(packagename))                  
    ...
    if (isDbgPkg) then
      do igroup = 1, NGROUP
        ...
        if (ncore .gt. 0) then
          do ibin = 1, NBIN
            write(binname, '(A, I2.2)') trim(shortname), ibin     
...
!          end if          
  ...
  subroutine carma_getcnstforbin(ielem, ibin, icnst)
    ...
    integer, intent(out) :: icnst       !! constituent index
 ...
 subroutine carma_accumulate_stats()
  ...
  subroutine carma_output_budget_diagnostics(state, ptend, dt, pname, old_cflux, cflux, flx_vap, flx_cnd, flx_ice, flx_sen)
    ...
    real(r8), intent(in), optional     :: old_cflux(pcols,pcnst)  !! cam_in%clfux from before the timestep_tend
    real(r8), intent(in), optional     :: cflux(pcols,pcnst)  !! cam_in%clfux from after the timestep_tend
    ...
    do i = 1, carma_ndiagpkgs
      if (trim(carma_diags_packages(i)) .eq. trim(pname)) then
        ...
        ! budgets related to physics packages other tahn CARMA
        call CARMA_OutputBudgetDiagnostics(carma, icnst4elem, icnst4gas, state, ptend, dt, pname, rc, old_cflux, cflux, flx_vap, flx_cnd, flx_ice, flx_sen)
        exit
      end if
    end do
...
!  subroutine carma_output_diagnostics(state, ptend, gpdiags, sbdiags, gsdiags, spdiags, bndiags)
  subroutine carma_output_diagnostics(state, ptend, pbuf, cam_in, gpdiags, sbdiags, gsdiags, spdiags, bndiags, nucdiags) !(LW: modified for homo. & hetero. nuc., Sep 2022)
    use cam_history, only: outfld
...
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ! LW (Sep 2022): added for homo. & hetero. nuc.
    real(r8), intent(in), dimension(pcols, pver, NELEM, NGROUP, NNUCDIAGS) :: nucdiags  !! CARMA nucleation diagnostic output
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    do igroup = 1, NGROUP 
      ...
      call outfld(trim(sname)//'NC', gpdiags(:, :, igroup, GPDIAGS_NC), pcols, lchnk)  !(LW 07/2023)
      call outfld(trim(sname)//'MR0', gpdiags(:, :, igroup, GPDIAGS_MR0), pcols, lchnk)!(LW 07/2023)

      call outfld(trim(sname)//'ADS', gpdiags(:, :, igroup, GPDIAGS_ADS), pcols, lchnk) 
      call outfld(trim(sname)//'VDS', gpdiags(:, :, igroup, GPDIAGS_VDS), pcols, lchnk) 
      call outfld(trim(sname)//'RES', gpdiags(:, :, igroup, GPDIAGS_RES), pcols, lchnk) 
      call outfld(trim(sname)//'MRS', gpdiags(:, :, igroup, GPDIAGS_MRS), pcols, lchnk) 
      call outfld(trim(sname)//'NDS', gpdiags(:, :, igroup, GPDIAGS_NDS), pcols, lchnk) 
      call outfld(trim(sname)//'ADM', gpdiags(:, :, igroup, GPDIAGS_ADM), pcols, lchnk) 
      call outfld(trim(sname)//'VDM', gpdiags(:, :, igroup, GPDIAGS_VDM), pcols, lchnk) 
      call outfld(trim(sname)//'REM', gpdiags(:, :, igroup, GPDIAGS_REM), pcols, lchnk) 
      call outfld(trim(sname)//'MRM', gpdiags(:, :, igroup, GPDIAGS_MRM), pcols, lchnk) 
      call outfld(trim(sname)//'NDM', gpdiags(:, :, igroup, GPDIAGS_NDM), pcols, lchnk) 
      call outfld(trim(sname)//'ADL', gpdiags(:, :, igroup, GPDIAGS_ADL), pcols, lchnk) 
      call outfld(trim(sname)//'VDL', gpdiags(:, :, igroup, GPDIAGS_VDL), pcols, lchnk) 
      call outfld(trim(sname)//'REL', gpdiags(:, :, igroup, GPDIAGS_REL), pcols, lchnk) 
      call outfld(trim(sname)//'MRL', gpdiags(:, :, igroup, GPDIAGS_MRL), pcols, lchnk) 
      call outfld(trim(sname)//'NDL', gpdiags(:, :, igroup, GPDIAGS_NDL), pcols, lchnk) 

!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      ! LW (Sep 2022): added for homo. & hetero. nuc.
      ! Output the nucleation statistics
      !call outfld(trim(sname)//'HOMA', nucdiags(:, :, 5, igroup,  NUCDIAGS_SO4CCN), pcols, lchnk)
      !call outfld(trim(sname)//'HETA', nucdiags(:, :, 6, igroup,  NUCDIAGS_DSTCCN), pcols, lchnk)
      !call outfld(trim(sname)//'HOMI', nucdiags(:, :, 5, igroup,  NUCDIAGS_SO4CCN), pcols, lchnk)
      !call outfld(trim(sname)//'HETI', nucdiags(:, :, 6, igroup,  NUCDIAGS_DSTCCN), pcols, lchnk)
      call outfld(trim(sname)//'HOMA', nucdiags(:, :, I_ELEM_CRCORE, igroup,  NUCDIAGS_SO4CCN), pcols, lchnk)
      call outfld(trim(sname)//'HETA', nucdiags(:, :, I_ELEM_CRDCOR, igroup,  NUCDIAGS_DSTCCN), pcols, lchnk)
      call outfld(trim(sname)//'HOMI', nucdiags(:, :, I_ELEM_CRCORE, igroup,  NUCDIAGS_SO4CCN), pcols, lchnk)
      call outfld(trim(sname)//'HETI', nucdiags(:, :, I_ELEM_CRDCOR, igroup,  NUCDIAGS_DSTCCN), pcols, lchnk)
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++    
    ...
    do igas = 1, NGAS
      ...
      call outfld(trim(cnst_name(icnst))//'C1', gsdiags(:, :, igas, GSDIAGS_C1), pcols, lchnk)
      call outfld(trim(cnst_name(icnst))//'C2', gsdiags(:, :, igas, GSDIAGS_C2), pcols, lchnk)
      call outfld(trim(cnst_name(icnst))//'C3', gsdiags(:, :, igas, GSDIAGS_C3), pcols, lchnk)
      call outfld(trim(cnst_name(icnst))//'T1', gsdiags(:, :, igas, GSDIAGS_T1), pcols, lchnk)
      call outfld(trim(cnst_name(icnst))//'T2', gsdiags(:, :, igas, GSDIAGS_T2), pcols, lchnk)
      call outfld(trim(cnst_name(icnst))//'T3', gsdiags(:, :, igas, GSDIAGS_T3), pcols, lchnk)
      call outfld(trim(cnst_name(icnst))//'AW1', gsdiags(:, :, igas, GSDIAGS_AW1), pcols, lchnk)
      call outfld(trim(cnst_name(icnst))//'AW2', gsdiags(:, :, igas, GSDIAGS_AW2), pcols, lchnk)
      call outfld(trim(cnst_name(icnst))//'AW3', gsdiags(:, :, igas, GSDIAGS_AW3), pcols, lchnk)
      call outfld(trim(cnst_name(icnst))//'F1', gsdiags(:, :, igas, GSDIAGS_F1), pcols, lchnk)
      call outfld(trim(cnst_name(icnst))//'F2', gsdiags(:, :, igas, GSDIAGS_F2), pcols, lchnk)
      call outfld(trim(cnst_name(icnst))//'F3', gsdiags(:, :, igas, GSDIAGS_F3), pcols, lchnk)
      call outfld(trim(cnst_name(icnst))//'LC',gsdiags(:, :, igas, GSDIAGS_LC),pcols, lchnk)
    end do
    ...
    ! Output the subgrid saturation factor.
    if (carma_do_grow) then
       call pbuf_get_field(pbuf, pbuf_get_index("QSATFAC"), qsatfac)
       call outfld('CRQSATFAC', qsatfac, pcols, lchnk)        
    end if              
    
    ! Allow models to output their own diagnostics
    call carma_OutputDiagnostics(carma, icnst4elem, state, ptend, pbuf, cam_in, rc, gpdiags)
  ...
  subroutine carma_wetdep_tend(state, ptend, dt,  pbuf, dlf, cam_out)
    ...
    real(r8), pointer, dimension(:,:)   :: prain_ptr  !(LW 08/2023: modified name to avoid confusion with the CARMA global var. 'prain')
    real(r8), pointer, dimension(:,:)   :: evapr_ptr  !(LW 08/2023: modified name to avoid confusion with the CARMA global var. 'nevapr')
    ...
    call pbuf_get_field(pbuf, pbuf_get_index('PRAIN'), prain_ptr)  !(LW 08/2023)
    call pbuf_get_field(pbuf, pbuf_get_index('NEVAPR'), evapr_ptr) !(LW 08/2023)
   ...
   cmfdqr(:ncol,:) = rprddp(:ncol,:)  + rprdsh(:ncol,:)
    ...
    call clddiag( state%t, state%pmid, state%pdel, cmfdqr, evapc, cldn, cldc, clds, cme, evapr_ptr, prain_ptr, &
        ...
        do ibin = 1, NBIN
          ...
          if (ibin <= maxbin) then
            ...
            do irh = 1, nrh
              ...
              do iwave = 1, NWAVE
!                write (carma % f_LUNOPRT, *) "CARMA mie calc:  start  " &
!                , igroup, ibin, iwave, carma % f_wave (iwave) &
!                , carma % f_group (igroup) % f_nmon (ibin)

                ! Using Mie code, calculate the optical properties:
...
!                write ( carma % f_LUNOPRT, *) "CARMA mie calc:  done  " &
!                , Qext, Qsca, asym
...
! ++++++++++++++++++++++++++++++++ (May 2022, LW) ++++++++++++++++++++++++++++++++++++++++
  !! This routine creates files containing optical properties for CARMA Liquid Clouds.
  !! These optical properties are used by the RRTMG radiation code to include 
  !! the impact of CARMA particles in the radiative transfer calculation.
  !!
  !! NOTE: The format of this file is determined by the needs of the radiative tranfer
  !! code, so ideally a routine would exist in that module that could create a file
  !! with the proper format. Since that doesn't exist, we do it all here.

  subroutine CARMA_CreateCloudOpticsFile(carma, rc)
    use radconstants, only : nswbands, nlwbands
    use wrap_nf
    !use wetr, only         : getwetr
    ...
    logical                             :: do_mie
    integer                             :: fid
    integer                             :: bindim, lwdim, swdim
    integer                             :: binvar, lwvar, swvar
    integer                             :: abs_lw_var
    integer                             :: ext_sw_var, ssa_sw_var, asm_sw_var
    integer                             :: dimids(2)
    real(kind=f)                        :: abs_lw(NBIN, nlwbands)
    real(kind=f)                        :: ext_sw(NBIN, nswbands)
    real(kind=f)                        :: ssa_sw(NBIN, nswbands)
    real(kind=f)                        :: asm_sw(NBIN, nswbands)
    character(len=8)                    :: c_name                   ! constituent name  
    character(len=255)                  :: filepath
    real(kind=f)                        :: rwet
    real(kind=f)                        :: Qext
    ...
    if (rc < 0) call endrun('carma_CreateCloudOpticsFile::CARMA_Get failed.')
    ...
    do igroup = 1, NGROUP
      ...
      call CARMAGROUP_Get(carma, igroup, rc, do_mie=do_mie, name=name, shortname=shortname, r=r, &
                          rlow=rlow, rup=rup, rmass=rmass, refidx=refidx, irhswell=irhswell, &
                          ienconc=ienconc, cnsttype=cnsttype, maxbin=maxbin)
      if (rc < 0) call endrun('carma_CreateCloudOpticsFile::CARMAGROUP_Get failed.')
      
      ! Are we supposed to do the mie calculation for this group?
      if ((do_mie) .and. (cnsttype == I_CNSTTYPE_PROGNOSTIC)) then
      
        call CARMAELEMENT_Get(carma, ienconc, rc, rho=rho)
        if (rc < 0) call endrun('carma_CreateCloudOpticsFile::CARMAELEMENT_Get failed.')
      
        ! A file needs to be created for all CARMA bins.
    
        !!!!! Create one optics file including all bins.
            filepath = trim(carma_model) // '_CRLIQ_rrtmg.nc'
            
            if (do_print) write(LUNOPRT,*) 'Creating CARMA Cloud optics file ... ', trim(filepath)
    
            ! Create the file.
            call wrap_create(filepath, NF90_CLOBBER, fid)
                          
            ! Define the dimensions: nbin, lwbands, swbands
            call wrap_def_dim(fid, 'nbin_idx',  NBIN,  bindim)
            call wrap_def_dim(fid, 'lw_band', nlwbands, lwdim)
            call wrap_def_dim(fid, 'sw_band', nswbands, swdim)

            write(LUNOPRT,*) "Defined bin_idx, lw_band, and sw_band dims."

            dimids(1) = bindim
            call wrap_def_var(fid, 'carmabin',  NF90_DOUBLE, 1, dimids(1:1), binvar)
            ...
            write(LUNOPRT,*) "Defined bin_idx, lw_band, and sw_band vars."
            
            call wrap_put_att_text(fid, binvar, 'units', 'm')   !+++ Note: should be [cm] here, instead of [m]
            call wrap_put_att_text(fid, lwvar, 'units', 'm') 
            call wrap_put_att_text(fid, swvar, 'units', 'm') 
  
            call wrap_put_att_text(fid, binvar, 'long_name', 'CARMA bins')
            call wrap_put_att_text(fid, lwvar, 'long_name', 'longwave bands')
            call wrap_put_att_text(fid, swvar, 'long_name', 'shortwave bands')
            ...
            call wrap_put_att_text(fid, sw_r_refidx_var, 'long_name', 'real refractive index of water - shortwave') 
            call wrap_put_att_text(fid, sw_i_refidx_var, 'long_name', 'imaginary refractive index of water - shortwave') 
            call wrap_put_att_text(fid, lw_r_refidx_var, 'long_name', 'real refractive index of water - longwave') 
            call wrap_put_att_text(fid, lw_i_refidx_var, 'long_name', 'imaginary refractive index of water - longwave') 
            ...
            ! Write out the dimensions.
            call wrap_put_var_realx(fid, binvar, r(:NBIN))      !+++ [cm]
            call wrap_put_var_realx(fid, lwvar, wave(:nlwbands) * 1e-2_f)       ![cm] -> [m]
            call wrap_put_var_realx(fid, swvar, wave(nlwbands+1:) * 1e-2_f)     ![cm] -> [m]
            ...
            do ibin = 1, NBIN
             ...
             if (ibin <= maxbin) then
              do iwave = 1, NWAVE
                write (carma % f_LUNOPRT, *) "CARMA mie calc:  start  " &
                , igroup, ibin, r(ibin), iwave, carma % f_wave (iwave) &
                , carma % f_group (igroup) % f_nmon (ibin)

                !  CB: Using Mie code, calculate the optical properties:
                ! 1) extinction coefficient
                ! 2) single scattering albedo
                ! 3) asymmetry factor.
                ! Assume the particle is homogeneous (no core).
                !
                ! NOTE: nmon, df, rmon and falpha are only used for fractal
                ! particles.
                ...
                call mie(carma, &
                         ...
                         asym, &
                         rc)
                if (rc < 0) call endrun('carma_CreateCloudOpticsFile::mie failed.')
                write ( carma % f_LUNOPRT, *) "CARMA mie calc:  done  " &
                , Qext, Qsca, asym

                ! Calculate  the shortwave and longwave properties?
                !
                ! NOTE: miess is in cgs units, but the optics file needs to be
                ! in mks units, so perform the necessary conversions.
                if (iwave <= nlwbands) then

                  ! Longwave just needs absorption: abs_lw.
                  abs_lw(ibin, iwave) = (Qext - Qsca) * PI * (r(ibin) * 1e-2_f)**2 / (rmass(ibin) * 1e-3_f)
                else
               
                  ! Shortwave needs extinction, single scattering albedo and asymmetry factor:
                  ! ext_sw, ssa_sw and asm_sw.
                  ext_sw(ibin, iwave - nlwbands) = Qext * PI * (r(ibin) * 1e-2_f)**2 / (rmass(ibin) * 1e-3_f)
                  ssa_sw(ibin, iwave - nlwbands) = Qsca / Qext
                  asm_sw(ibin, iwave - nlwbands) = asym
                  
                end if   !if (iwave <= nlwbands)
              end do   !do iwave = 1, NWAVE
            end if   !if (ibin <= maxbin)
          end do   !do ibin = 1, NBINq
            
            ! Write out the longwave fields.
            ret = nf90_put_var (fid, abs_lw_var, abs_lw(:NBIN, :))
            if (ret/=NF90_NOERR) then
               write(iulog,*)'CARMA_CreateCloudOpticsFile: error writing varid =', abs_lw_var
               call handle_error (ret)
            end if
            
            ! Write out the shortwave fields.
            ret = nf90_put_var (fid, ext_sw_var, ext_sw(:NBIN, :))
            if (ret/=NF90_NOERR) then
               write(iulog,*)'CARMA_CreateCloudOpticsFile: error writing varid =', ext_sw_var
               call handle_error (ret)
            end if
            ret = nf90_put_var (fid, ssa_sw_var, ssa_sw(:NBIN, :))
            if (ret/=NF90_NOERR) then
               write(iulog,*)'CARMA_CreateCloudOpticsFile: error writing varid =', ssa_sw_var
               call handle_error (ret)
            end if
            ret = nf90_put_var (fid, asm_sw_var, asm_sw(:NBIN, :))
            if (ret/=NF90_NOERR) then
               write(iulog,*)'CARMA_CreateCloudOpticsFile: error writing varid =', asm_sw_var
               call handle_error (ret)
            end if
            
            ! Close the file.
            call wrap_close(fid)
      end if    !if cnsttype == I_CNSTTYPE_PROGNOSTIC then
    end do    !do igroup = 1, NGROUP
    
    return
  end subroutine CARMA_CreateCloudOpticsFile 
! ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  !! Convert individual tracer fields or tracer tendencies from wet mixing ratios
  !! to dry mixing ratios. This is needed because CARMA wants wet mixing ratios, but
  !! the tracers in state may be dry mixing ratios.
  !!
  !! @author  Chuck Bardeen
  !! @version Jun 2023
  function CARMA_SetWetToDry(state, icol, q, cnst_idx)

    implicit none 

    real(r8), dimension(pver)          :: CARMA_SetWetToDry
    type(physics_state), intent(in)    :: state     !! physics state variables
    integer, intent(in)                :: icol      !! column index
    real(r8), intent(in)               :: q(pver)   !! data to be scaled
    integer, intent(in), optional      :: cnst_idx  !! constituent index
    
    ! If it is a consituent, check to see if it should be converted back
    ! to dry or left as wet.
    CARMA_SetWetToDry(:) = q(:)
    
    if (present(cnst_idx)) then
      if (cnst_type(cnst_idx) .eq. 'dry') then
        CARMA_SetWetToDry(:) = CARMA_SetWetToDry(:) * state%pdel(icol,:) / state%pdeldry(icol,:)
      end if
    else
      CARMA_SetWetToDry(:) = CARMA_SetWetToDry(:) * state%pdel(icol,:) / state%pdeldry(icol,:)
    end if
    ...
    return
  end function CARMA_SetWetToDry
